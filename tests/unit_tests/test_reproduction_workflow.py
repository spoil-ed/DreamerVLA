...
def test_dockerfile_pins_runtime_source_and_complete_third_party_install() -> None:
    ...
    assert (
        "ARG MUJOCO_SYSTEM_IMAGE=simat-acr-bj-registry.cn-beijing.cr.aliyuncs.com/"
        "cloud-platform/mujoco-system:cu128-ubuntu2204-v1" in text
    )
    assert "FROM ${MUJOCO_SYSTEM_IMAGE}" in text
    assert "WORKDIR /opt/dreamervla" in text
    assert "DVLA_DATA_ROOT=/data" in text
    assert "LIBERO_CONFIG_PATH=/tmp/libero" in text
    assert "LIBERO_CONFIG_PATH=/data/.libero" not in text
    assert "bash scripts/install_env.sh" in text
    assert "env.INSTALL_APT_TOOLS=false" in text
    assert "INSTALL_OPENVLA_OFT_THIRD_PARTY=true" in text
    assert "github.com/moojink/transformers-openvla-oft/archive/" in text
    assert "uv pip check" in text
    assert "python -m dreamervla.diagnostics.verify_install" in text
    assert ".vla-image.json" in text
    assert "USER sim" in text
...
def test_dockerfile_inherits_the_mujoco_rendering_system_layer() -> None:
    text = (PROJECT_ROOT / "docker" / "Dockerfile").read_text(encoding="utf-8")

    assert "FROM ${MUJOCO_SYSTEM_IMAGE}" in text
    assert "nvidia/cuda:" not in text


def test_system_compose_exposes_complete_mujoco_uv_environment() -> None:
    compose = (PROJECT_ROOT / "docker" / "system" / "compose.yaml").read_text(encoding="utf-8")
    smoke = (PROJECT_ROOT / "docker" / "system" / "smoke" / "vla.sh").read_text(encoding="utf-8")

    assert "mujoco-vla:" in compose
    assert "mujoco-vla:cu124-h100-v1" in compose
    assert "dockerfile: docker/Dockerfile" in compose
    assert "third_party_source: ../../third_party" in compose
    assert "DEPENDENCY_SOURCE_STAGE: third-party-local" in compose
    assert "MUJOCO_SYSTEM_IMAGE" in compose
    assert "mujoco-vla-smoke:" in compose
    assert 'command: ["/usr/local/bin/smoke-mujoco-vla"]' in compose
    assert "uv pip check" in smoke
    assert "python -m dreamervla.diagnostics.verify_install" in smoke


def test_third_party_install_can_reuse_pinned_local_checkouts() -> None:
    third_party = (PROJECT_ROOT / "scripts" / "install" / "40_third_party.sh").read_text(
        encoding="utf-8"
    )
    special = (PROJECT_ROOT / "scripts" / "install" / "50_special_packages.sh").read_text(
        encoding="utf-8"
    )

    assert 'USE_PRESEEDED_THIRD_PARTY="${USE_PRESEEDED_THIRD_PARTY:-0}"' in third_party
    assert 'USE_PRESEEDED_THIRD_PARTY="${USE_PRESEEDED_THIRD_PARTY:-0}"' in special
    assert "Missing preseeded third_party/LIBERO checkout." in third_party
    assert "Missing preseeded third_party/openvla-oft checkout." in third_party
    assert "Missing preseeded third_party/egl_probe checkout." in special
...
def test_container_install_pins_third_party_runtime_compatibility() -> None:
    ...
    assert "chardet==5.2.0" in third_party_install
