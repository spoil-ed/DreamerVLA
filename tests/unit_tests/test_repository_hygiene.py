...
def test_docs_and_smoke_script_do_not_point_at_removed_entrypoints() -> None:
    ...
    smoke_script = (
        project_root / "scripts" / "preprocess" / "smoke_pretokenize.sh"
    ).read_text(encoding="utf-8")
    dreamervla_data_script = (
        project_root / "scripts" / "prepare_dreamervla_data.sh"
    ).read_text(encoding="utf-8")
    ...
    assert (
        project_root / "LIBERO" / "benchmark_scripts" / "download_libero_datasets.py"
    ).is_file()
