...
def test_default_artifact_paths_follow_dvla_data_root(
    ...
    from dreamervla.diagnostics.benchmarks import wm_single_episode_overfit as diag
    ...
    assert args.wm_ckpt is None
    assert args.classifier_ckpt is None
...
def test_check_requires_explicit_run_artifacts(monkeypatch: pytest.MonkeyPatch) -> None:
    from dreamervla.diagnostics.benchmarks import wm_single_episode_overfit as diag

    monkeypatch.setattr("sys.argv", ["wm_single_episode_overfit", "--stage", "check"])
