...
def test_manual_cotrain_resume_contract_is_replay_free() -> None:
    manual_notes = (ROOT / "spec" / "99_manual_notes.md").read_text(
        encoding="utf-8"
    )
...
def test_active_route_docs_match_hydra_runner_targets() -> None:
    ...
    route_reference = (ROOT / "docs" / "reference" / "routes.md").read_text(
        encoding="utf-8"
    )
    tutorial = (
    ...
    assert (
        "`openvla_onetraj_libero_cotrain` | `CotrainRunner`" in route_reference
    )
