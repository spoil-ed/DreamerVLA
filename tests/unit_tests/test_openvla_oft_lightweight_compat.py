...
def test_official_openvla_oft_default_root_lives_under_third_party() -> None:
    from dreamer_vla.diagnostics.openvla_oft_obs_action_policy import default_openvla_oft_root
...
def test_lightweight_action_tokenizer_action_head_and_collator_interfaces() -> None:
    ...
    action, hidden_c, hidden_d = head.predict_action_with_intermediates(hidden)

    assert action.shape == (2, NUM_ACTIONS_CHUNK, ACTION_DIM)
    assert hidden_c.shape[:2] == (2, NUM_ACTIONS_CHUNK)
    assert hidden_d.shape[:2] == (2, NUM_ACTIONS_CHUNK)
