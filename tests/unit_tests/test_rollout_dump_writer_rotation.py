...
def _steps(T: int, emb_dim: int = 8) -> list[dict]:
    out = []
    for t in range(T):
        out.append(
            {
...
def _write(reward, hidden, *, demos_per_shard, start_index=0, n=5, prefix="r0_shard"):
...
def test_rotates_every_n_demos_and_restarts_demo_index(tmp_path):
    reward, hidden = tmp_path / "reward", tmp_path / "hidden"
    _write(reward, hidden, demos_per_shard=2, n=5)
...
def test_every_shard_is_independently_readable_with_metadata(tmp_path):
    reward, hidden = tmp_path / "reward", tmp_path / "hidden"
    _write(reward, hidden, demos_per_shard=2, n=5)
    ...
    for name in sorted(p.name for p in reward.glob("*.hdf5")):
        ...
        with h5py.File(str(hidden / name), "r") as hf:
            for key in hf["data"]:
                assert hf["data"][key]["obs_embedding"].shape[1] == 8


def test_start_index_offsets_shard_names_for_resume(tmp_path):
    reward, hidden = tmp_path / "reward", tmp_path / "hidden"
    _write(reward, hidden, demos_per_shard=2, start_index=3, n=3)
...
def test_seed_replay_reads_rotated_shards(tmp_path):
