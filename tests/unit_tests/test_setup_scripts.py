...
def test_setup_and_download_scripts_are_release_entrypoints() -> None:
    ...
    assert "download_libero_datasets.py" not in download_step_text
    assert "physical-intelligence/libero" in download_step_text
    assert "a4336d589d589045d1c56423ffdf3b88a0e19b1f" in download_step_text
    assert "--repo-type dataset" in download_step_text
    assert "-u http_proxy" in download_step_text
    assert "LIBERO_SUITES" not in download_cfg_text
...
def test_uv_cuda_training_profiles_are_locked_and_mutually_exclusive() -> None:
    root = _project_root()
    pyproject = tomllib.loads((root / "pyproject.toml").read_text(encoding="utf-8"))
    groups = pyproject["dependency-groups"]
    uv = pyproject["tool"]["uv"]

    assert set(groups["cu124-train"]) == {
        "flash-attn==2.7.1.post1",
        "torch==2.5.1",
        "torchaudio==2.5.1",
        "torchvision==0.20.1",
    }
    assert set(groups["cu130-train"]) == {
        "torch==2.13.0",
        "torchvision==0.28.0",
    }
    assert [
...
def test_portable_data_layout_manifest_exists_and_is_linked() -> None:
    ...
    assert "${DVLA_DATA_ROOT}/datasets/lerobot/physical-intelligence/libero" in manifest_text
