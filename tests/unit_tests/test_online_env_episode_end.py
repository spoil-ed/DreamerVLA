...
def test_online_env_wrappers_use_shared_episode_end_logic() -> None:
    ...
    libero_online_env = (repo / "src/env/libero_online_env.py").read_text(encoding="utf-8")
...
def test_online_training_script_separates_episode_horizon_from_training_budget() -> None:
    repo = Path(__file__).resolve().parents[1]
    script = (repo / "scripts/train_online_pi0_action_hidden_dreamervla.py").read_text(encoding="utf-8")
