...
class _FakePrefixInputEncoder:
    def encode_raw_observation_prefix_input_batch(self, observations, *, text_mode):
        batch_size = len(observations)
        assert text_mode == "exact"
        return SimpleNamespace(
            latent=torch.ones(batch_size, 968, 2048),
            attention_mask=torch.ones(batch_size, 968, dtype=torch.bool),
        )
...
def test_pi05_rgb_replay_carries_prefix_input_attention_mask(tmp_path: Path) -> None:
    ...
    replay = Pi05TrajectoryReplay(
        data_dir=reward,
        sequence_length=3,
        encoder=_FakePrefixInputEncoder(),
        encode_batch_size=2,
        rank=0,
        world_size=1,
        seed=3,
        task_ids=(0,),
        rotate_images_180=False,
        encoder_method="encode_raw_observation_prefix_input_batch",
        encoder_kwargs={"text_mode": "exact"},
    )
