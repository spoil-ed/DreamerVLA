...
def test_workflow_task_flag_dispatches_only_selected_suite(
    ...
    remaining, override = normalize_task_flag(
        ...
        hydra_key="tasks" if config_name != "preprocess/preprocess_suite" else "task",
...
def test_download_workflow_rejects_suite_task_flag() -> None:
    from dreamervla.launchers.workflow import main

    with pytest.raises(SystemExit, match="--task is not supported"):
        main(["--config-name", "download/config", "--task", "libero_goal", "dry_run=true"])
