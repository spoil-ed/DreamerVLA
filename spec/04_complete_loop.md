...
训练 run root 为 `outputs/<实验名>/<时间戳>/`。完整可恢复状态写入平铺的
`checkpoints/latest.ckpt`，基于真实评估 success rate 选出的文件命名为
`epoch=<完成 epoch>-accuracy=<value>.ckpt`。评估输入既可给该 run root，也可给
`checkpoints/` 或具体 `.ckpt`；评估产物固定写入 `outputs/eval/<任务名>/`。
