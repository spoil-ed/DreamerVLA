# OpenVLA-OFT Hidden-Token Mainline Convergence

## Objective

The only OpenVLA-OFT world-model observation latent on the DreamerVLA mainline is
the current-frame projected vision hidden-token embedding:
...
- Hydra metadata namespace: `task.openvla_oft.hidden_token.*`.
- Task path field: `task.openvla_oft.hidden_token_dir`.
- Sidecar metadata value: `obs_hidden_source: hidden_token`.
- Sidecar directory suffix: `_oft_hidden_token_vla_policy_h1`.
...
The public contract uses `hidden_token` consistently. Generic `hidden_dir`
transport names remain valid where replay pairs role-based `reward/` and
`hidden/` channels independently of the concrete latent source.
...
1. Use `task.openvla_oft.hidden_token.*` consistently in every mainline
   collector, warmup, sync-cotrain, and Ray-cotrain route.
2. Make all four supported LIBERO task configs (`goal`, `object`, `spatial`, and
   `10`) declare the same one-image, history-one, 256-token hidden-token
...
4. Rename the OpenVLA offline preprocessing entry point to the hidden-token role
   and make it emit only `hidden_token` sidecars. Remove its C/D and
   action-query/56-token sidecar outputs.
5. Name projected-token helpers and metadata with the `hidden_token` role.
...
7. Keep other VLA families and unrelated side routes. Their own latent formats
   must not be renamed to OpenVLA hidden tokens.
...
3. Collector and offline-preprocessor tests emit
   `obs_hidden_source=hidden_token` and `[T,256,4096]` storage.
...
5. Repository hygiene checks find no OpenVLA WM/sidecar configuration that uses
   56 tokens or exposes any retired public naming surface.
...
- Rebuilding expensive hidden-token sidecars that already satisfy the canonical
  schema.
