<!-- omit in toc -->
# Contributing to DreamerVLA

Thanks for taking the time to contribute to DreamerVLA! ❤️
...
All contributions (including the project team's contribution) takes the form of [GitHub Pull Requests](https://github.com/spoil-ed/DreamerVLA/pulls).
To contribute, first you need to [fork the repository](https://github.com/spoil-ed/DreamerVLA/fork) and clone it to your local machine.
...
The commit subject must use DreamerVLA's bracketed type format and stay within
72 characters:
```
[<type>]: <description>
```
...
- `init`: repository-baseline initialization
...
* Note that not all CI checks will be executed immediately due to limited computational resources. The reviewer will add `run-ci` label to the PR when the PR is ready to merge or a full CI run is needed. For PRs from existing DreamerVLA team members, please make sure to remove the `run-ci` label if you only wish to make minor changes that do not require a full CI run (e.g., fixing typos in documentation).
