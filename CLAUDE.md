...
Install (LIBERO editable install, flash-attn wheel, ColossalAI / TensorNVMe / APEX, egl_probe): see [docs/install.md](docs/install.md). Rendering: set `MUJOCO_GL=egl`; smoke-test via `python -m dreamer_vla.diagnostics.smoke_libero_online_env`. NCCL / CUDA timeouts under DDP are usually one rank diverging (NaN, mismatched batch) — read the rank-0 log before assuming network; the DDP synchronization guards in `dreamer_vla/algorithms/ppo/outcome.py` exist for a reason, don't remove them.
