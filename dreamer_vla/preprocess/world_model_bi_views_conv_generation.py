...
def main():
    ...
    parser.add_argument(
        ...
        "-T",
        type=str,
        default="spatial",  # Default based on the example base_dir
    ...
    print(
        f"Output files will use task_name: '{task_name_for_filename}' and resolution: {resolution_for_filename}"
    )
    ...
    for task_id, task in enumerate(task_list):
        ...
        for i, trj in enumerate(trj_list):
            ...
            for j in range(num_frames):
                ...
                img_third_view_file = os.path.join(
                    imgs_third_view_path, f"image_{j}.png"
                )
                img_wrist_file = os.path.join(imgs_wrist_path, f"image_{j}.png")

                # Check if files exist to ensure data integrity for this frame
                if (
                    os.path.exists(action_file)
                    and os.path.exists(img_third_view_file)
                    and os.path.exists(img_wrist_file)
                ):
                    ...
                    print(
                        f"Warning: Missing file(s) for frame {j} in trajectory {trj_path}. Stopping processing for this trajectory."
                    )
            ...
            if len(img_third_view_list) < his + 1:
                print(
                    f"Warning: Trajectory {trj_path} has only {len(img_third_view_list)} frames, which is insufficient for 'his'={his} (requires at least {his + 1} frames). Skipping this trajectory."
                )
            ...
            for j in range(len(action_list) - 1):
                ...
                img_c_h = copy.deepcopy(
                    img_third_view_list[max(j - his + 1, 0) : j + 1]
                ) + copy.deepcopy(img_wrist_list[max(j - his + 1, 0) : j + 1])
                ...
                img_c_f = copy.deepcopy(
                    img_third_view_list[j + 1 : j + 2]
                ) + copy.deepcopy(img_wrist_list[j + 1 : j + 2])

                prompt_text = f"Finish the task: {task.replace('_', ' ')}."
                conv = {
                    "task_type": "world",
                    "task_name": task,
                    ...
                    "conversations": [
                        ...
                        },
                        {
                            "from": "gpt",
                            "value": "<|image|>"
                            * len(img_c_f),  # The model generates a single image
                        },
                    ],
                    ...
                    "action": action_c,
                    "state": [],
                    "input_image": copy.deepcopy(img_c_h),
                    "input_action": copy.deepcopy(action_c),
                    "input_state": [],
                    "target_image": copy.deepcopy(img_c_f),
                    "target_action": [],
                    "target_state": [],
                }
    ...
    print(
        f"Total conversations generated: {len(train_convs) + len(val_convs_ind) + len(val_convs_ood)}"
    )
