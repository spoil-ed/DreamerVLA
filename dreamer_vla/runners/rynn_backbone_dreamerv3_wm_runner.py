...
from dreamer_vla.models.encoder.protocol import EncoderInputBatch
from dreamer_vla.runners.dreamerv3_pixel_runner import DreamerV3PixelRunner
...
class RynnBackboneDreamerV3WMRunner(DreamerV3PixelRunner):
    ...
    def _setup_auxiliary_modules(self) -> None:
        ...
        if OmegaConf.select(self.cfg, "dataset.hidden_dir", default=None) is not None:
            self._print(
                "[dreamerv3-pixel] using precomputed Rynn hidden; skipping backbone encoder build."
            )
            return
    ...
    def _encode_pixel_observations(
        ...
        if self.rynn_encoder is None:
            raise RuntimeError(
                "RynnVLA encoder must be built before encoding pixel observations"
            )
        if images.ndim != 5:
        ...
        if channels != 6:
            raise ValueError(
                f"Rynn backbone pixel WM expects two RGB views concatenated as C=6, got C={channels}"
            )
        ...
        chunk_size = max(
            1, int(OmegaConf.select(self.cfg, "training.encoder_chunk_size", default=8))
        )
        ...
        for start in range(0, len(prompt_text), chunk_size):
            ...
            hidden_chunks.append(
                self.rynn_encoder.encode_inputs(encoder_batch).hidden.detach()
            )
    ...
    def _prepare_batch_for_model(
        ...
        if not isinstance(images, torch.Tensor):
            raise ValueError(
                "Rynn backbone pixel WM batch must contain tensor `images`."
            )
        ...
        for key, value in list(prepared.items()):
            ...
            if isinstance(value, torch.Tensor):
                prepared[key] = value.to(device=self.device, non_blocking=True)
        prepared["obs_embedding"] = obs_embedding.to(
    ...
    def _maybe_save_viz(
        ...
        if not all(
            ...
            for x in (images, obs_embedding, actions, is_first)
        ):
            return
            ...
            for view_idx, label in enumerate(view_labels):
                ...
                panels.extend(
                    [
                        (
                            f"{label} cur",
                            self._tensor_to_pil(images_viz[sample_idx, 0, c0:c1]),
                        ),
                        (
                            f"{label} recon",
                        ...
                        (
                            f"{label} next",
                            self._tensor_to_pil(images_viz[sample_idx, 1, c0:c1]),
                        ),
                        (
                            f"{label} prior",
