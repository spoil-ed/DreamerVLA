# WM hidden_decoder variants v4 → v4-E

All v4 / v4-D / v4-E world-model pretraining runs over the **legacy** pi0 action-head (35×1024 = 35840-dim) hidden reconstruction target. All start from the v3 ckpt `v3_e7_ft_perwindow.ckpt` except where noted.
...
| **v4-D Pi0xform L4** | `launch_wm_v4D_pi0xform.sh` | `..._v4d_pi0xform.yaml` | `Pi0StyleHiddenDecoder` in `dreamerv3_torch.py` | transformer decoder with learned queries |
| **v4-D Pi0xform deep** | `launch_wm_v4D_pi0xform_deep.sh` | `..._v4d_pi0xform_deep.yaml` | (same) | deeper transformer |
| **v4-D V2-mid** | `launch_wm_v4D_v2mid.sh` | (override-only) | — | encoder compresses 35840→8192 |
...
| **v4-E Pi0xform resurrect** | `launch_wm_v4E_pi0xform_resurrect.sh` | (override-only) | (uses existing Pi0StyleHiddenDecoder) | re-run after the v4-D Pi0xform was killed |
| **v4-F Time-broadcast** | `launch_wm_v4F_time_broadcast.sh` | `..._v4f_time_broadcast.yaml` | `Pi0TimeBroadcastDecoder` in `dreamerv3_torch.py` | 5 time queries + broadcast over 7 joints (hard tie); see `docs/hidden_token_structure_report.md` |
...
| `hidden_decoder_kind` | Class | Params (for 10240 → 35840) |
|---|---|---|
| `mlp` | `MLPHead` (plain) | ~400M (L2×8192) — ~2.9B (L4×16384) |
| `resnet` | `ResMLPHead` (skip connections) | ~900M (L4×8192) — ~2.9B (L4×16384) |
| `pi0_transformer` | `Pi0StyleHiddenDecoder` (learned queries + transformer) | ~80M (L4 d_model=1024 mem=8) |
| `per_token_mlp` | `PerTokenMLPHead` (shared MLP across 35 tokens) | **~27M** (L2×2048 q=128) ⭐ |
| `pi0_time_broadcast` | `Pi0TimeBroadcastDecoder` (5 time queries + broadcast over 7 joints) | ~80M; same as `pi0_transformer` but attention seq 13 vs 43 (-91% attn ops) |
