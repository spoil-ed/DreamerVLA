...
def main() -> None:
    ...
    world_model = hydra.utils.instantiate(cfg.world_model, hidden_dim=hidden_dim)
    ...
    for idx in indices:
        ...
        with torch.no_grad():
            latent = world_model.encode_latent(cur_pooled_b)
            prior_next = world_model.predict_next(latent, action)
            pred_next_hidden = world_model.transition_head(prior_next.feature())  # [1, D]
    ...
    summary = {
        ...
        "prediction": {
            k: _mean([m["prediction"][k]        for m in all_metrics])
            for k in ("mse", "cosine", "relative_error")
        },
        "identity_baseline": {
            k: _mean([m["identity_baseline"][k] for m in all_metrics])
            for k in ("mse", "cosine", "relative_error")
        },
