...
from scripts.train_online_pi0_action_hidden_dreamervla import (
    build_encoder, load_world_model_state, obs_to_action_hidden,
)
...
def parse_args():
    ...
    p.add_argument("--config", default=str(PROJECT_ROOT / "configs/dreamer_vla_libero_goal_pi0_action_hidden_head_actor.yaml"))
    ...
    p.add_argument("--use-sft-init", action="store_true",
...
def main():
    ...
    load_world_model_state(world_model, args.world_model_ckpt, reset_reward_head=(rwh == "binary"))
    # Override WM with trained state from --ckpt
    world_model.load_state_dict(ckpt["state_dicts"]["world_model"], strict=False)
    freeze_module(world_model); world_model.eval()
    ...
    freeze_module(policy); policy.eval()
    ...
    env = DreamerVLAOnlineTrainEnv(
        ...
        task_id=int(args.task_id), task_ids=(int(args.task_id),),
        seed=args.seed, max_steps=int(args.episode_horizon),
        action_input="normalized", task_sampling="sequential",
        init_state_sampling="sequential", history_length=2, include_state=True,
        vla_rotate_180=True, obs_hidden_source="action_query", action_head_type="pi0_query",
    )

    obs, _ = env.reset(task_id=int(args.task_id), episode_id=int(args.episode_id),
                       seed=int(args.seed) + int(args.task_id)*100 + int(args.episode_id))

    # ── Storage ─────────────────────────────────────────────────────────────
    real_actions, real_rewards, real_obs_embs = [], [], []
    real_term, real_trunc = False, False
    ...
    for t in range(int(args.episode_horizon)):
        with torch.no_grad():
            obs_emb = obs_to_action_hidden(encoder, processor, obs, device, args.target_token_id)
            ...
            if is_first:
                ...
                cur_latent = world_model({
                    "mode": "observe_next", "latent": cur_latent,
                    "hidden": obs_emb, "actions": prev_wm_action, "is_first": False,
                })
            real_latents.append(cur_latent)
            real_imag_rewards.append(float(world_model.state_reward(cur_latent).float().cpu().item()))

            feat = world_model({"mode": "actor_input", "latent": cur_latent}).float()
            ach, _, _ = policy({"mode": "sample", "hidden": feat, "deterministic": True, "return_chunk": True})
            policy_action = ach.reshape(-1, ach.shape[-1])[0, :7].detach().cpu().float().numpy()
        ...
        prev_wm_action = torch.from_numpy(wm_a).to(device=device, dtype=obs_emb.dtype).unsqueeze(0)
        obs = next_obs
        if terminated or truncated:
            real_term, real_trunc = terminated, truncated
            break

    T_real = len(real_actions)
    real_success = bool(real_term)
    print(f"[real] T={T_real}  success={real_success}  sum_reward={sum(real_rewards):.3f}", flush=True)
    ...
    for t in range(T_real):
        with torch.no_grad():
            ...
            _, _, extra = policy({"mode": "sample", "hidden": feat, "deterministic": True, "return_chunk": True})
            ach = extra["action_chunk"]
            a0 = ach.reshape(-1, ach.shape[-1])[0, :7].float()
            imag_actions.append(a0.detach().cpu().tolist())
            # WM predict_next
            cur = world_model({"mode": "predict_next", "latent": cur, "actions": a0.to(dtype=torch.bfloat16).unsqueeze(0)})
    ...
    print(f"{'t':>4} {'cos(emb)':>10} {'a_MAE':>8} {'a_max':>8} {'real_r':>8} {'imag_r':>8} {'real_postr':>11}")
    idxs = sorted(set([0, T_real//8, T_real//4, T_real//2, 3*T_real//4, T_real-10, T_real-5, T_real-1]))
    for t in idxs:
        if t < 0 or t >= T_real: continue
        print(f"{t:>4} {cos_emb[t]:>10.4f} {action_mae_per_step[t]:>8.4f} {action_max_per_step[t]:>8.4f} {real_rewards[t]:>8.4f} {imag_rewards[t]:>8.4f} {real_imag_rewards[t]:>11.4f}")

    fire_imag = int(np.argmax(np.array(imag_rewards) > 0.5)) if (np.array(imag_rewards) > 0.5).any() else -1
    fire_real_post = int(np.argmax(np.array(real_imag_rewards) > 0.5)) if (np.array(real_imag_rewards) > 0.5).any() else -1
    print(f"\nimag fire@>0.5: step={fire_imag}  (final imag_r={imag_rewards[-1]:.4f})")
    print(f"real-posterior fire@>0.5: step={fire_real_post}  (final={real_imag_rewards[-1]:.4f})")
    print(f"real env success={real_success}  (sum_real_reward={sum(real_rewards):.3f})")
    print(f"cos(real,recon): mean={cos_emb.mean():.4f}  min={cos_emb.min():.4f}  last10_mean={cos_emb[-10:].mean():.4f}")
    print(f"action MAE:       mean={action_mae_per_step.mean():.4f}  max={action_mae_per_step.max():.4f}")

    out = {
        "ckpt": args.ckpt,
        "task_id": args.task_id, "episode_id": args.episode_id,
        "T": int(T_real),
        "real_success": bool(real_success),
        "real_actions": real_actions, "real_rewards": real_rewards,
        "imag_actions": imag_actions, "imag_rewards": imag_rewards,
        ...
        "fire_imag": fire_imag, "fire_real_post": fire_real_post,
    }
    out_json = Path(args.out_json) if args.out_json else Path(args.ckpt).parent.parent / "real_vs_imagine.json"
