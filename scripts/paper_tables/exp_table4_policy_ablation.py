...
VARIANTS = {
    "bc_only": {
        ...
        "overrides": ["training.run_actor_critic_phase=false", "algorithm.actor_bc_to_ref_scale=1.0"],
    },
    "imagined_ppo": {
        "label": "Imagined PPO",
        "env": {"RUN_TAG": "table4_imagined_ppo"},
        "overrides": ["algorithm.update_type=wmpo_ppo", "algorithm.kl_coef=0.0", "algorithm.actor_bc_to_ref_scale=0.0"],
    },
    "kl_regularized": {
        "label": "+ KL regularization",
        "env": {"RUN_TAG": "table4_kl_regularized"},
        "overrides": ["algorithm.update_type=wmpo_ppo", "algorithm.kl_coef=0.03", "algorithm.actor_bc_to_ref_scale=0.0"],
    },
    "bc_anchor": {
        "label": "+ BC anchor",
        "env": {"RUN_TAG": "table4_bc_anchor"},
        "overrides": ["algorithm.update_type=wmpo_ppo", "algorithm.kl_coef=0.03", "algorithm.actor_bc_to_ref_scale=0.03"],
    },
}
...
def cmd_plan(args: argparse.Namespace) -> int:
    for key, spec in VARIANTS.items():
        ...
        code = run_or_print(["bash", "scripts/train_dreamer_vla.sh", *spec["overrides"]], env=env, execute=args.execute)
        if code:
            return code
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan.add_argument("--out-dir", default="data/outputs/dreamervla/table4_policy_ablation")
    ...
    collect.add_argument("--out-json", default=str(DEFAULT_OUTPUTS_DIR / "policy_ablation.json"))
    collect.add_argument("--out-tex", default=str(DEFAULT_TABLES_DIR / "policy_ablation.tex"))
