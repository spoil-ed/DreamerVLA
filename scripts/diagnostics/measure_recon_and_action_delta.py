...
def main():
    ...
    print(
        f"global per-step MSE:    mean={per_step_mse.mean():.4g}  p50={np.median(per_step_mse):.4g}  p90={np.percentile(per_step_mse, 90):.4g}  max={per_step_mse.max():.4g}"
    )
    print(
        f"global cosine sim:      mean={per_step_cos.mean():.4f}  p50={np.median(per_step_cos):.4f}  p10={np.percentile(per_step_cos, 10):.4f}  min={per_step_cos.min():.4f}"
    )
    print(
        f"global max-abs-diff:    mean={per_step_max_abs.mean():.3f}  max={per_step_max_abs.max():.3f}"
    )
    print(
        f"relative L2 error:      mean={rel_err.mean():.4f}  p50={np.median(rel_err):.4f}  max={rel_err.max():.4f}"
    )
    print(
        f"real obs_emb L2 norm:   mean={real_norm.mean():.2f}  recon L2 norm: mean={recon_norm.mean():.2f}"
    )

    # ── B. SFT actor: real vs recon input → action delta ────────────────────
    with torch.no_grad():
        # All T steps in one batch each
        _, _, sft_real_extra = sft_policy(
            {
                "mode": "sample",
                "hidden": real.to(dtype=torch.bfloat16),
                "deterministic": True,
            }
        )
        _, _, sft_recon_extra = sft_policy(
            {
                "mode": "sample",
                "hidden": recon.to(dtype=torch.bfloat16),
                "deterministic": True,
            }
        )
    ...
    print(
        f"per-step MSE:     mean={per_step_action_mse.mean():.5g}  p50={np.median(per_step_action_mse):.5g}  p90={np.percentile(per_step_action_mse, 90):.5g}  max={per_step_action_mse.max():.5g}"
    )
    print(
        f"per-step MAE:     mean={per_step_action_mae.mean():.5g}  p50={np.median(per_step_action_mae):.5g}  max={per_step_action_mae.max():.5g}"
    )
    print(
        f"per-step max|Δ|:  mean={per_step_action_max.mean():.4f}  max={per_step_action_max.max():.4f}"
    )
    ...
    print(
        f"SFT(real)  vs demo:  MAE mean={diff_real_vs_demo.abs().mean().item():.5g}  max|Δ| mean={diff_real_vs_demo.abs().max(-1).values.mean().item():.4f}"
    )
    print(
        f"SFT(recon) vs demo:  MAE mean={diff_recon_vs_demo.abs().mean().item():.5g}  max|Δ| mean={diff_recon_vs_demo.abs().max(-1).values.mean().item():.4f}"
    )

    # ── Per-step printout at a few representative timesteps ────────────────
    print("\n=========== Per-step samples ===========")
    print(
        f"{'t':>4} {'recon_mse':>10} {'recon_cos':>10} {'a_real-recon MAE':>17} {'a_real-demo MAE':>16} {'a_recon-demo MAE':>17}"
    )
    for t in [0, 10, 20, 40, 60, 80, T - 5, T - 1]:
        ...
        print(
            f"{t:>4} {per_step_mse[t]:>10.4g} {per_step_cos[t]:>10.4f} {ar_ac:>17.5g} {ar_demo:>16.5g} {ac_demo:>17.5g}"
        )
    ...
    out_json = (
        Path(args.out_json)
        if args.out_json
        else Path(args.ckpt).parent.parent / "recon_and_action_delta.json"
    )
