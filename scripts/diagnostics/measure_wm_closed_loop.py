...
Usage:
  python scripts/measure_wm_closed_loop.py \
    --ckpt <wm_ckpt> \
...
def parse_args():
    ...
    p.add_argument("--actor-cfg", required=True,
...
def slice_dict_latent(latent: dict[str, torch.Tensor], t: int) -> dict[str, torch.Tensor]:
    return {k: v[:, t] if v.ndim >= 2 else v for k, v in latent.items()}


@torch.no_grad()
def imagine_path(world_model, actor_or_none, posterior_latent, demo_actions,
                 start_step: int, T: int, mode: str):
    ...
    for t in range(start_step, T - 1):
        if mode == "env":
            ...
            _, _, extra = actor_or_none({"mode": "sample", "hidden": hidden_in, "deterministic": True})
...
def stats_vs_post(imag_hidden: torch.Tensor, post_hidden: torch.Tensor,
                  imag_reward: np.ndarray, sparse: np.ndarray) -> dict[str, float]:
    ...
    return {
        ...
        "reward_acc": acc,
        "reward_pred_pos_mean": float(pr[sp > 0.5].mean()) if (sp > 0.5).any() else float("nan"),
        "reward_pred_neg_mean": float(pr[sp <= 0.5].mean()) if (sp <= 0.5).any() else float("nan"),
...
def main():
    ...
    wm = hydra.utils.instantiate(cfg.world_model).to(device=device, dtype=torch.bfloat16)
    missing, unexpected = wm.load_state_dict(ckpt["model"], strict=False)
    print(f"[load] wm tensors={len(ckpt['model'])}  missing={len(missing)}  unexpected={len(unexpected)}")
    wm.eval()
    ...
    for fname in args.files:
        ...
        for start in args.start_steps:
            if start >= T - 1:
                continue
            rec: dict[str, Any] = {"file": fname, "start": start, "T": T, "horizon": T - start - 1}
            for mode in ["env", "sft_open_loop", "closed_loop"]:
                imag_h, imag_r = imagine_path(wm, actor, post_latent, act_t, start, T, mode)
                rec[mode] = stats_vs_post(imag_h, post_hidden[start:], imag_r, sparse[start:])
            print(f"  {fname}  start={start}  closed_loop cos_mean={rec['closed_loop']['cos_mean']:.4f}  "
