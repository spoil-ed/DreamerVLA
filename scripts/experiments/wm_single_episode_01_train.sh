#!/usr/bin/env bash
# Stage 01: train the single-episode WM overfit checkpoint, without eval.
set -euo pipefail
...
echo "[wm-single-episode-train] DVLA_ROOT      = ${DVLA_ROOT}" >&2
echo "[wm-single-episode-train] DVLA_DATA_ROOT = ${DVLA_DATA_ROOT} (${_DVLA_DATA_ROOT_SRC})" >&2
...
"${PYTHON_BIN}" -m dreamervla.diagnostics.wm_single_episode_overfit \
  --stage train \
  --run \
  --report-steps "${REPORT_STEPS:-1200}" \
  "$@"
