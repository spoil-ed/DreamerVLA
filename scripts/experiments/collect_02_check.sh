#!/usr/bin/env bash
# Stage COLLECT-02: summarize collected replay and write collection_manifest.json.
set -euo pipefail
...
echo "[collect-output] DVLA_ROOT      = ${DVLA_ROOT}" >&2
echo "[collect-output] DVLA_DATA_ROOT = ${DVLA_DATA_ROOT} (${_DVLA_DATA_ROOT_SRC})" >&2
...
"${PYTHON_BIN}" -m dreamervla.diagnostics.experiment_stage_checks collect-output \
