#!/usr/bin/env bash
# Stage COLLECT-01: run cold-start rollout collection into unified replay dirs.
set -euo pipefail
...
echo "[collect-run] DVLA_ROOT      = ${DVLA_ROOT}" >&2
echo "[collect-run] DVLA_DATA_ROOT = ${DVLA_DATA_ROOT} (${_DVLA_DATA_ROOT_SRC})" >&2
...
"${PYTHON_BIN}" -m dreamervla.diagnostics.experiment_stage_checks collect-run \
