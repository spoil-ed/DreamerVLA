...
#
# Mirror example:
#   HF_ENDPOINT=https://hf-mirror.com CALVIN_DOWNLOAD_METHOD=hf_shards bash scripts/download/30_calvin_dataset.sh
set -euo pipefail
...
if [[ "${CALVIN_DOWNLOAD_METHOD}" == "official" || "${CALVIN_DOWNLOAD_METHOD}" == "curl" || "${CALVIN_DOWNLOAD_METHOD}" == "freiburg" ]]; then
  if ! command -v curl >/dev/null 2>&1; then
    echo "curl is required for CALVIN_DOWNLOAD_METHOD=${CALVIN_DOWNLOAD_METHOD}." >&2
    exit 2
  fi
  ...
  echo "[download:30_calvin_dataset] hf_endpoint=${HF_ENDPOINT:-<default>}"
  ...
  if ! command -v openxlab >/dev/null 2>&1; then
    echo "openxlab is required for CALVIN_DOWNLOAD_METHOD=opendatalab; run: pip install openxlab" >&2
    exit 2
  fi
