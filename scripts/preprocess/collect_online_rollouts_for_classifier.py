...
These shards drop in as the ``rollout_raw_dir`` / ``rollout_hidden_dir`` source
in ``configs/wmpo_classifier_libero_goal_v4_with_rollouts.yaml``. The dataset
derives ``finish_step`` from ``dones`` and ``complete`` from ``rewards`` per
episode — matching WMPO's ``meta["finish_step"]`` / ``meta["complete"]``
semantics.
