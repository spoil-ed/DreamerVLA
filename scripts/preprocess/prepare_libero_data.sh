...
RUN_OFT_ACTION_HIDDEN="${RUN_OFT_ACTION_HIDDEN:-0}"
RUN_VALIDATE="${RUN_VALIDATE:-1}"
export RUN_MARKED RUN_REWARD RUN_PRETOKENIZE RUN_ACTION_HIDDEN RUN_INPUT_TOKEN_HIDDEN RUN_OFT_ACTION_HIDDEN RUN_VALIDATE
...
if [[ "${RUN_INPUT_TOKEN_HIDDEN}" == "1" ]]; then
  PREPROCESS_STEPS+=("32_input_token_hidden.sh")
fi
if [[ "${RUN_OFT_ACTION_HIDDEN}" == "1" ]]; then
  PREPROCESS_STEPS+=("35_oft_action_hidden.sh")
fi
