...
if [[ "${OVERWRITE}" != "1" && -d "${OFT_HIDDEN_TOKEN_DIR}" ]]; then
  if python -m dreamervla.preprocess.check_artifacts command=hdf5-dir \
    ...
    echo "[10_oft_hidden_token] skip hidden-token sidecar: ${OFT_HIDDEN_TOKEN_DIR}"
    exit 0
  fi
  echo "[10_oft_hidden_token] repair incomplete hidden-token sidecar: ${OFT_HIDDEN_TOKEN_DIR}" >&2
fi
...
print(
    "[10_oft_hidden_token] openvla_oft_root="
