...
if [[ -z "${raw_hdf5}" ]]; then
  ...
  echo "The default 20_libero_dataset step installs LeRobot data, not raw HDF5." >&2
  echo "For the libero_goal OpenVLA reproduction, run scripts/reproduce/01_prepare_assets.sh." >&2
  exit 2
fi
...
if [[ "${OVERWRITE}" != "1" && -n "${filtered_hdf5}" ]]; then
  if python -m dreamervla.preprocess.check_artifacts command=hdf5-dir dir="${HDF5_DIR}" reference_dir="${MARKED_DIR}"; then
    echo "[00_hdf5_reward] skip filter: ${HDF5_DIR}"
  else
    echo "[00_hdf5_reward] repair incomplete filtered stage: ${HDF5_DIR}" >&2
    rm -rf "${HDF5_DIR}"
...
if [[ "${OVERWRITE}" != "1" && -n "${reward_hdf5}" ]]; then
  if python -m dreamervla.preprocess.check_artifacts command=hdf5-dir dir="${REWARD_DIR}" reference_dir="${HDF5_DIR}"; then
    echo "[00_hdf5_reward] skip reward: ${REWARD_DIR}"
  else
    echo "[00_hdf5_reward] repair incomplete reward stage: ${REWARD_DIR}" >&2
    rm -rf "${REWARD_DIR}"
