...
def _predict_intermediates_chunk(
    *,
    ...
    for index in range(start, end):
        images = [
            _image_from_hdf5(
                obs_group, key, hidx, rotate_images_180=bool(args.rotate_images_180)
            )
    ...
    with torch.inference_mode():
        from openvla_oft.constants import ACTION_DIM, NUM_ACTIONS_CHUNK, IGNORE_INDEX
...
def _write_source_sidecars(
    *,
    ...
    tmp_c = (
        ...
        else out_c_path.with_name(f"{out_c_path.name}.rank{rank}.tmp")
    )
    tmp_d = (
        None
        if out_d_path is None
        else out_d_path.with_name(f"{out_d_path.name}.rank{rank}.tmp")
    )
    tmp_action = (
        None
    ...
    with ExitStack() as stack:
        source = stack.enter_context(
            h5py.File(source_path, "r", swmr=True, libver="latest")
        )
        out_c = (
            None
            if tmp_c is None
            else stack.enter_context(h5py.File(tmp_c, "w", libver="latest"))
        )
        out_d = (
            None
            if tmp_d is None
            else stack.enter_context(h5py.File(tmp_d, "w", libver="latest"))
        )
        out_action = (
            None
        ...
        out_action_data = (
            None if out_action is None else out_action.create_group("data")
        )
        ...
        for demo_key in tqdm(
            demo_keys, desc=f"rank{rank} {source_path.name}", leave=False
        ):
            ...
            demo_action = (
                None
                if out_action_data is None
                else out_action_data.create_group(demo_key)
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=str(
            PROJECT_ROOT
            / "data/processed_data/libero_goal_no_noops_t_256_oft_action_hidden_c_h8"
        ),
    )
    parser.add_argument(
        "--out-d-dir",
        default=str(
            PROJECT_ROOT
            / "data/processed_data/libero_goal_no_noops_t_256_oft_action_hidden_d_h8"
        ),
    )
    parser.add_argument("--out-action-dir", default=None)
    parser.add_argument("--skip-cd-sidecars", action="store_true")
    parser.add_argument(
        "--oft-ckpt", default=str(PROJECT_ROOT / "data/ckpts/OpenVLA-OFT/libero_goal")
    )
...
def main() -> None:
    ...
    out_action_dir = (
        None if args.out_action_dir is None else _project_path(args.out_action_dir)
    )
    ...
    if rank == 0:
        ...
        base_config["out_action_dir"] = (
            None if out_action_dir is None else str(out_action_dir)
        )
    ...
    for source_path in assigned:
        ...
        existing_paths = [
            path
            for path in (out_c_path, out_d_path, out_action_path)
            if path is not None
        ]
