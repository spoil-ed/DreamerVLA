...
#
# Attach:
#   tmux attach -t ppo_alltasks_multienv_g67
set -euo pipefail

cd "$(dirname "$0")/.."

PYTHON="${PYTHON:-/home/user01/miniconda3/envs/dreamervla/bin/python}"
SESSION="${SESSION:-ppo_alltasks_multienv_g67}"
...
NUM_ENVS_PER_RANK="${NUM_ENVS_PER_RANK:-2}"

TS="${TS:-$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-/mnt/data/spoil/workspace/DreamerVLA/data/outputs/dreamervla/online_wmpo_outcome/alltasks_frozenlegacy_ppo_g67_multienv${NUM_ENVS_PER_RANK}_trace_wmclf_online_replay3k_freezeenc_globalcov_wmrefresh75_long300_chunk5/${TS}}"
LOG_FILE="${LOG_FILE:-/mnt/data/spoil/workspace/DreamerVLA/data/outputs/logs/ppo_alltasks_multienv${NUM_ENVS_PER_RANK}_trace_wmclf_replay3k_freezeenc_globalcov_chunk5_g67_${TS}.log}"
...
echo "[run_online_wmpo_alltasks] task_ids=$TASK_IDS total_env_steps=$TOTAL_ENV_STEPS max_train_updates=$MAX_TRAIN_UPDATES collect_chunk_steps=$COLLECT_CHUNK_STEPS num_envs_per_rank=$NUM_ENVS_PER_RANK"
...
tmux new-session -d -s "$SESSION" -c "$PWD" \
  ...
  --num-envs-per-rank $NUM_ENVS_PER_RANK \
  --log-every $LOG_EVERY \
