...
from dreamer_vla.algorithms.dreamer_vla import imagine_actor_critic_step, world_model_pretrain_step
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Experimental multi-env collector: run this many LIBERO envs per rank and batch "
             "their VLA encoder calls. The original single-env script is unchanged.",
    )
...
def obs_batch_to_action_hidden(
    ...
    if len(observations) == 1:
        return obs_to_action_hidden(encoder, processor, observations[0], device, target_token_id)

    token_rows: list[list[int]] = []
    for obs in observations:
        tokens = processor.process_item(obs["vla_record"], training_mode=False)
        if isinstance(tokens, tuple):
            tokens = tokens[0]
        token_rows.append([int(tok) for tok in tokens])
    labels = [[-100] * len(row) for row in token_rows]
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message=r".*nested_from_padded CUDA kernels only support.*")
        warnings.filterwarnings("ignore", message=r".*PyTorch API of nested tensors is in prototype stage.*")
        _, _, _, hidden_states, _, _, _ = encoder.backbone(
...
class OnlineReplay:
    def __init__(
        ...
        *,
        ...
        self.task_ids = tuple(int(task_id) for task_id in task_ids) if task_ids is not None else None
    ...
    def episodes(self) -> list[dict[str, Any]]:
        return [record for task_id in sorted(self.episodes_by_task) for record in self.episodes_by_task[task_id]]
    ...
    def _valid_records(self) -> list[dict[str, Any]]:
        return [
            record for records in self.episodes_by_task.values()
    ...
    def task_stats(self, task_ids: tuple[int, ...] | None = None) -> dict[str, dict[str, int]]:
        ...
        for record in self._valid_records():
            ...
            if requested is not None and task_id not in requested:
                continue
            entry = stats.setdefault(task_id, {
                "episodes": 0,
                "successes": 0,
                "failures": 0,
                "transitions": 0,
    ...
    def sample(self, batch_size: int) -> dict[str, torch.Tensor]:
        ...
        action_dim = int(np.asarray(windows[0][0]["wm_action"], dtype=np.float32).reshape(-1).shape[0])
        ...
        for batch_idx, window in enumerate(windows):
            for time_idx in range(self.sequence_length):
                current_actions[batch_idx, time_idx] = np.asarray(window[time_idx]["wm_action"], dtype=np.float32)
    ...
    def sample_classifier_windows(
        ...
        *,
        ...
        candidates = [
            record for record in self._valid_records()
    ...
    def classifier_window_count(self, *, window: int, chunk_size: int) -> int:
        ...
        return sum(
            1 for record in self._valid_records()
            if int(record.get("finish_step", len(record["episode"]))) >= window_env
        )
...
def save_checkpoint(
    ...
    *,
    ...
    if classifier_optimizer is not None:
        payload["state_dicts"]["classifier_optimizer"] = classifier_optimizer.state_dict()
    ...
    *,
    ...
    for key, module in modules.items():
        if key in state_dicts:
            ...
            missing, unexpected = _unwrap(module).load_state_dict(state_dicts[key], strict=use_strict)
...
def main() -> None:
    ...
    episode_log_f = open(out_dir / "logs" / f"episodes_rank{rank}.jsonl", "a", encoding="utf-8")
    train_update_log_f = open(out_dir / "logs" / f"train_updates_rank{rank}.jsonl", "a", encoding="utf-8")
    ppo_log_f = open(out_dir / "logs" / f"ppo_groups_rank{rank}.jsonl", "a", encoding="utf-8")
    ...
    if is_rank0:
        ...
        print(
            ...
            f"num_envs_per_rank={args.num_envs_per_rank}",
            flush=True,
        )
    ...
    if is_dist:
        world_model = DDP(
            world_model, device_ids=[local_rank], output_device=local_rank,
    ...
    if args.actor_update_kind == "outcome":
        ...
        cls_payload = torch.load(args.classifier_ckpt, map_location="cpu", weights_only=False)
        cls_config_blob = cls_payload.get("config", {}).get("classifier")
        if cls_config_blob is None:
            raise RuntimeError(f"classifier ckpt {args.classifier_ckpt} has no config.classifier blob")
    ...
    num_envs_per_rank = max(1, int(args.num_envs_per_rank))
    global_collectors = max(1, int(world_size) * num_envs_per_rank)
    envs: list[DreamerVLAOnlineTrainEnv] = []
    obs_list: list[dict[str, Any]] = []
    next_task_positions: list[int] = []
    for env_idx in range(num_envs_per_rank):
        collector_idx = int(rank) * num_envs_per_rank + env_idx
        env_seed = int(args.seed) + int(rank) * 100000 + env_idx * 1000
        env_i = DreamerVLAOnlineTrainEnv(
            task_suite_name=args.task_suite,
            task_id=task_ids[collector_idx % len(task_ids)],
            task_ids=task_ids,
            seed=env_seed,
        ...
        obs_i, _info = env_i.reset(seed=env_seed, task_id=task_ids[collector_idx % len(task_ids)])
        envs.append(env_i)
        obs_list.append(obs_i)
        next_task_positions.append(collector_idx + global_collectors)
    replay = OnlineReplay(
    ...
    episodes: list[list[dict[str, Any]]] = [[] for _ in range(num_envs_per_rank)]
    episode_returns = [0.0 for _ in range(num_envs_per_rank)]
    episode_lens = [0 for _ in range(num_envs_per_rank)]
    latents: list[Any | None] = [None for _ in range(num_envs_per_rank)]
    prev_wm_actions: list[torch.Tensor | None] = [None for _ in range(num_envs_per_rank)]
    pending_policy_actions_list: list[deque[np.ndarray]] = [deque() for _ in range(num_envs_per_rank)]
    pending_chunk_ids = [0 for _ in range(num_envs_per_rank)]
    pending_chunk_indices = [0 for _ in range(num_envs_per_rank)]
    pending_chunk_lens = [0 for _ in range(num_envs_per_rank)]
    ...
    batched_obs_embeddings: torch.Tensor | None = None
    try:
        for env_step in range(int(resume_env_step) + 1, int(args.total_env_steps) + 1):
            if stop_training:
                break
            final_env_step = int(env_step)
            env_slot = (int(env_step) - 1) % num_envs_per_rank
            if env_slot == 0 or batched_obs_embeddings is None:
                batched_obs_embeddings = obs_batch_to_action_hidden(
                    encoder,
                    processor,
                    obs_list,
                    device,
            ...
            env = envs[env_slot]
            obs = obs_list[env_slot]
            episode = episodes[env_slot]
            episode_return = float(episode_returns[env_slot])
            episode_len = int(episode_lens[env_slot])
            latent = latents[env_slot]
            prev_wm_action = prev_wm_actions[env_slot]
            pending_policy_actions = pending_policy_actions_list[env_slot]
            pending_chunk_id = int(pending_chunk_ids[env_slot])
            pending_chunk_index = int(pending_chunk_indices[env_slot])
            pending_chunk_len = int(pending_chunk_lens[env_slot])
            obs_embedding = batched_obs_embeddings[env_slot : env_slot + 1]
            ...
            with torch.no_grad():
                ...
                if not pending_policy_actions:
                    ...
                    action_chunk, _log_prob, _extra = policy({
                        "mode": "sample",
                        "hidden": feat,
                        "deterministic": bool(args.deterministic_collect) or not bool(args.stochastic_collect),
                        "return_chunk": True,
                    })
                    chunk_np = (
                    ...
                    collect_chunk_steps = max(1, min(collect_chunk_steps, int(chunk_np.shape[0])))
            ...
            episodes[env_slot] = episode
            episode_returns[env_slot] = float(episode_return)
            episode_lens[env_slot] = int(episode_len)
            latents[env_slot] = latent
            prev_wm_actions[env_slot] = prev_wm_action
            pending_chunk_ids[env_slot] = int(pending_chunk_id)
            pending_chunk_indices[env_slot] = int(pending_chunk_index)
            pending_chunk_lens[env_slot] = int(pending_chunk_len)
            ...
            if bool(args.global_coverage_train_start):
                ...
                if is_dist:
                    ready_t = torch.tensor([int(local_basic_ready)], device=device, dtype=torch.long)
                    dist.all_reduce(ready_t, op=dist.ReduceOp.MIN)
                    all_ranks_basic_ready = bool(int(ready_t.item()))
                all_ranks_train_ready = bool(last_global_coverage_ready and all_ranks_basic_ready)
            else:
                all_ranks_train_ready = bool(local_train_ready)
                if is_dist:
                    ready_t = torch.tensor([int(local_train_ready)], device=device, dtype=torch.long)
                    dist.all_reduce(ready_t, op=dist.ReduceOp.MIN)
            ...
            if is_dist and args.train_every is None and all_ranks_train_ready:
                ...
                updates_t = torch.tensor([local_num_updates], device=device, dtype=torch.long)
            ...
            if num_updates > 0:
                for _ in range(num_updates):
                    ...
                    in_wm_refresh = wm_refresh_target > 0 and wm_refresh_updates < wm_refresh_target
                    ...
                    classifier_metrics: dict[str, Any] = {
                        "loss": 0.0, "acc": 0.0, "precision": 0.0, "recall": 0.0,
                        "f1": 0.0, "pos_frac": 0.0, "prob_mean": 0.0,
                    }
                    if do_actor_phase:
                        last_phase = "ppo"
                        obs_for_update = {
                        ...
                        if args.actor_update_kind == "outcome":
                            ...
                            if ppo_log_f is not None:
                                _prev_params_flat = torch.cat(
                                    [p.detach().float().flatten() for p in policy_module.parameters() if p.requires_grad]
                                )
                            ...
                            if ppo_log_f is not None:
                                ...
                                _drift_max = float(_delta.abs().max().item()) if _delta.numel() > 0 else 0.0
                                _drift_rel = _drift_l2 / max(_prev_norm, 1e-12)
                                _start_points_per_window = int(ac_metrics.get(
                                    "wmpo/start_points_per_window",
                                    int(batch["obs_embedding"].shape[1]),
                                ))
                                _num_groups = int(ac_metrics.get("wmpo/num_groups", 0))
                                _batch_episode_ids = batch["episode_ids"].detach().cpu().tolist()
                                _batch_collection_indices = batch["collection_indices"].detach().cpu().tolist()
                                _batch_task_episode_indices = batch["task_episode_indices"].detach().cpu().tolist()
                                _batch_episode_lengths = batch["episode_lengths"].detach().cpu().tolist()
                                _batch_sample_limits = batch["sample_limits"].detach().cpu().tolist()
                                _batch_source_ranks = batch["source_ranks"].detach().cpu().tolist()
                                _batch_task_ids = batch["task_ids"].detach().cpu().tolist()
                                _batch_start_indices = batch["start_indices"].detach().cpu().tolist()
                                _batch_episode_success = batch["episode_success"].detach().cpu().tolist()
                                ...
                                for _group_idx in range(_num_groups):
                                    _batch_idx = _group_idx // max(1, _start_points_per_window)
                                    _time_offset = _group_idx % max(1, _start_points_per_window)
                                    if _batch_idx >= len(_batch_task_ids):
                                        break
                                    _window_start = int(_batch_start_indices[_batch_idx])
                                    _group_source_episode_ids.append(int(_batch_episode_ids[_batch_idx]))
                                    _group_source_collection_indices.append(int(_batch_collection_indices[_batch_idx]))
                                    _group_source_task_episode_indices.append(int(_batch_task_episode_indices[_batch_idx]))
                                    _group_source_ranks.append(int(_batch_source_ranks[_batch_idx]))
                                    _group_source_task_ids.append(int(_batch_task_ids[_batch_idx]))
                                    _group_source_episode_success.append(bool(_batch_episode_success[_batch_idx]))
                                    _group_source_episode_lengths.append(int(_batch_episode_lengths[_batch_idx]))
                                    _group_source_sample_limits.append(int(_batch_sample_limits[_batch_idx]))
                                    _group_source_window_starts.append(_window_start)
                                    _group_source_time_offsets.append(int(_time_offset))
                                    _group_source_absolute_start_indices.append(_window_start + int(_time_offset))
                                _entry = {
                                    ...
                                    "env_step": int(env_step),
                                    "update_step": int(update_step + 1),  # this update hasn't bumped yet
                                    "success_rate_all_rollouts": float(ac_metrics.get("wmpo/success_rate", 0.0)),
                                    "num_groups": _num_groups,
                                    "group_size": int(ac_metrics.get("wmpo/group_size", 0)),
                                    "start_points_per_window": _start_points_per_window,
                                    "num_all_success_groups": int(ac_metrics.get("wmpo/num_all_success_groups", 0)),
                                    "num_all_fail_groups": int(ac_metrics.get("wmpo/num_all_fail_groups", 0)),
                                    "num_mixed_groups": int(ac_metrics.get("wmpo/num_mixed_groups", 0)),
                                    "group_success_rates": list(ac_metrics.get("wmpo/group_success_rates", [])),
                                    "group_success_counts": list(ac_metrics.get("wmpo/group_success_counts", [])),
                                    "group_rollout_successes": list(ac_metrics.get("wmpo/group_rollout_successes", [])),
                                    "group_finish_steps": list(ac_metrics.get("wmpo/group_finish_steps", [])),
                                    "group_has_variance": list(ac_metrics.get("wmpo/group_has_variance", [])),
                                    ...
                                    "actor_loss": float(ac_metrics.get("actor_loss", 0.0)),
                                    "actor_grad_norm": float(ac_metrics.get("actor_grad_norm", 0.0)),
                                }
                    ...
                    if bool(args.update_classifier_online) and classifier is not None and classifier_optimizer is not None:
                        cls_module = _unwrap(classifier)
                        cls_ready_local = replay.classifier_window_count(
                            window=int(cls_module.cfg.window),
                            chunk_size=int(getattr(cls_module.cfg, "chunk_size", 1)),
                        ) > 0
                        cls_ready = bool(cls_ready_local)
                        if is_dist:
                            cls_ready_t = torch.tensor([int(cls_ready_local)], device=device, dtype=torch.long)
                            dist.all_reduce(cls_ready_t, op=dist.ReduceOp.MIN)
                            cls_ready = bool(int(cls_ready_t.item()))
                        if cls_ready:
                            cls_metrics_list = []
                            for _cls_update in range(max(1, int(args.classifier_updates_per_train))):
                                cls_metrics_list.append(online_classifier_update_step(
                                    classifier=classifier,
                                    optimizer=classifier_optimizer,
                                    replay=replay,
                                    device=device,
                                    batch_size=int(args.classifier_batch_size),
                                    early_neg_stride=int(args.classifier_early_neg_stride),
                                    grad_clip=float(args.classifier_grad_clip),
                                ))
                            classifier_metrics = {
                                "loss": float(np.mean([item["loss"] for item in cls_metrics_list])),
                                "acc": float(np.mean([item["acc"] for item in cls_metrics_list])),
                                "precision": float(np.mean([item["precision"] for item in cls_metrics_list])),
                                "recall": float(np.mean([item["recall"] for item in cls_metrics_list])),
                                "f1": float(np.mean([item["f1"] for item in cls_metrics_list])),
                                "pos_frac": float(np.mean([item["pos_frac"] for item in cls_metrics_list])),
                                "prob_mean": float(np.mean([item["prob_mean"] for item in cls_metrics_list])),
                                "grad_norm": float(np.mean([item["grad_norm"] for item in cls_metrics_list])),
                    ...
                    train_update_entry = {
                        ...
                        "batch_episode_ids": batch["episode_ids"].detach().cpu().tolist(),
                        "batch_collection_indices": batch["collection_indices"].detach().cpu().tolist(),
                        "batch_task_episode_indices": batch["task_episode_indices"].detach().cpu().tolist(),
                        "batch_source_ranks": batch["source_ranks"].detach().cpu().tolist(),
                        "batch_task_ids": batch["task_ids"].detach().cpu().tolist(),
                        "batch_window_start_indices": batch["start_indices"].detach().cpu().tolist(),
                        "batch_episode_success": batch["episode_success"].detach().cpu().tolist(),
                        "batch_episode_lengths": batch["episode_lengths"].detach().cpu().tolist(),
                        "batch_sample_limits": batch["sample_limits"].detach().cpu().tolist(),
                        "batch_reward_sums": batch["rewards"].detach().cpu().sum(dim=1).tolist(),
                        "batch_terminal_counts": batch["is_terminal"].detach().cpu().sum(dim=1).tolist(),
                        "batch_done_counts": batch["dones"].detach().cpu().sum(dim=1).tolist(),
            ...
            if is_rank0 and update_step > 0 and update_step % args.save_every == 0 and update_step != last_saved_update:
                save_checkpoint(
                    out_dir,
            ...
            if done:
                ...
                if episode_log_f is not None:
                    ...
                    episode_entry = {
                        ...
                        "collect_chunk_index_counts": dict(Counter(collect_chunk_indices)),
                        "collect_chunk_len_counts": dict(Counter(collect_chunk_lens)),
                        "last_collect_chunk_index": (
                            None if not collect_chunk_indices else int(collect_chunk_indices[-1])
                        ),
                    }
                ...
                next_task_id = int(task_ids[next_task_positions[env_slot] % len(task_ids)])
                next_task_positions[env_slot] += global_collectors
                obs, _info = env.reset(task_id=next_task_id)
                obs_list[env_slot] = obs
                episodes[env_slot] = []
                episode_returns[env_slot] = 0.0
                episode_lens[env_slot] = 0
                latents[env_slot] = None
                prev_wm_actions[env_slot] = None
                pending_policy_actions.clear()
                pending_chunk_indices[env_slot] = 0
                pending_chunk_lens[env_slot] = 0
            else:
                obs_list[env_slot] = next_obs
