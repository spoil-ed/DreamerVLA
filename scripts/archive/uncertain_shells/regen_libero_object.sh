#!/usr/bin/env bash
# All preprocessing for libero_object: success sidecar + failure demo replay + failure sidecar.
#
# Phases (sequential):
...
#   CUDA_VISIBLE_DEVICES=6,7  NUM_GPUS=2  MASTER_PORT=29561  MAX_RETRIES=3
#
...
#   CUDA_VISIBLE_DEVICES=4,5 NUM_GPUS=2 bash scripts/regen_libero_object.sh
set -uo pipefail

SUITE=object
HORIZON=5          # libero_object uses action horizon = 5
EXPECTED_SUCCESS=10
DEFAULT_PORT=29561
