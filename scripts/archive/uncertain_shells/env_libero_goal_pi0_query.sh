...
export PI0_LEGACY_HIDDEN_DIR="${PI0_LEGACY_HIDDEN_DIR:-${PROJECT_ROOT}/data/processed_data/libero_goal_no_noops_t_256_pi0_legacy_action_hidden_vla_policy_h2}"
# Route the canonical RYNN_HIDDEN_DIR by ACTION_HEAD_TYPE so preprocess and
# WM/actor configs all agree: pi0_query -> 5120-d sidecar, legacy -> 35840-d
# legacy full-action-hidden sidecar (35 tokens * 1024).
if [[ "${ACTION_HEAD_TYPE}" == "legacy" ]]; then
  export RYNN_HIDDEN_DIR="${RYNN_HIDDEN_DIR:-${PI0_LEGACY_HIDDEN_DIR}}"
else
  export RYNN_HIDDEN_DIR="${RYNN_HIDDEN_DIR:-${PI0_QUERY_HIDDEN_DIR}}"
fi
export RYNN_HIDDEN_FULLSEQ_DIR="${RYNN_HIDDEN_FULLSEQ_DIR:-${RYNN_HIDDEN_DIR}}"
