#!/usr/bin/env bash
# All preprocessing for libero_spatial: success sidecar + failure demo replay + failure sidecar.
#
# Phases (sequential):
...
#   CUDA_VISIBLE_DEVICES=6,7  NUM_GPUS=2  MASTER_PORT=29571  MAX_RETRIES=3
#
...
#
# Usage:
#   bash scripts/regen_libero_spatial.sh
#   CUDA_VISIBLE_DEVICES=4,5 NUM_GPUS=2 bash scripts/regen_libero_spatial.sh
set -uo pipefail

SUITE=spatial
HORIZON=10         # libero_spatial uses action horizon = 10
EXPECTED_SUCCESS=10
DEFAULT_PORT=29571
