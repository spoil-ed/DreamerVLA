#!/usr/bin/env bash
# V3a-deep: ResNet hidden_decoder, 6 layers × 8192 units. GPU 6.
set +e
...
RUN=wm_pretrain_legacy_v4D_resnet_L6u8192_gpu7_${TS}
...
echo "===== ${RUN} start ====="; date
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node=1 --master-port=29517 -m dreamer_vla.cli.train \
