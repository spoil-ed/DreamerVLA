#!/usr/bin/env bash
# V2-mid: encoder compresses 35840 → 8192 (current is Identity passthrough).
# Plain MLP decoder (same as v4-B). GPU 6.
set +e
...
RUN=wm_pretrain_legacy_v4D_v2mid_emb8192_gpu6_${TS}
...
echo "===== ${RUN} start ====="; date
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node=1 --master-port=29512 -m dreamer_vla.cli.train \
  ...
  world_model.embed_dim=8192 \
  world_model.hidden_decoder_kind=mlp \
