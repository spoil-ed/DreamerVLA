...
export CUDA_VISIBLE_DEVICES=${TSSM_GPU:-5}
...
RUN=wm_pretrain_legacy_v4D_tssm_concatO_L6d600w32_gpu${TSSM_GPU:-5}_${TS}
...
echo "===== ${RUN} start ====="; date
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node=1 --master-port=${TSSM_PORT:-29514} -m dreamer_vla.cli.train \
  --config-name dreamer_vla_libero_goal_pi0_legacy_action_hidden_head_actor_v4d_tssm \
  ...
  dataloader.batch_size=${TSSM_BATCH_SIZE:-96} \
  dataloader.num_workers=${TSSM_NUM_WORKERS:-10} \
