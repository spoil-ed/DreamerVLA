...
Usage:
    python scripts/eval_chunkwm_closeloop.py \
...
def load_chunk_wm(ckpt_path: str, device: torch.device) -> ChunkAwareRynnDinoWMWorldModel:
    sd = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    wm_cfg_blob = sd.get("cfg", {}).get("world_model", {})
    chunk_size = int(wm_cfg_blob.get("chunk_size", 5))
    kwargs = {}
    for k in (
        "obs_dim", "action_dim", "token_count", "token_dim", "model_dim",
        "depth", "heads", "mlp_dim", "dropout", "num_hist", "num_pred",
        "max_seq_len", "hidden_loss_scale", "cosine_loss_scale",
        "rollout_loss_scale", "rollout_horizon", "rollout_context",
        "reward_head_type", "reward_loss_scale", "reward_hidden_dim",
        "reward_init_logit", "reward_pos_weight", "return_predictions",
    ):
        if k in wm_cfg_blob:
    ...
    print(f"[load] global_step={sd.get('global_step')} epoch={sd.get('epoch')}"
          f" missing={missing} unexpected={unexpected}")
    return wm.eval().to(device)


def load_demo(raw_p: Path, hid_p: Path, demo_key: str) -> tuple[np.ndarray, np.ndarray] | None:
...
def rollout(
    ...
    for c in range(N):
        ...
        if mode == "open":
            ...
            new_action_history = torch.zeros(1, H, wm.action_dim, device=obs.device, dtype=obs.dtype)
            if H > 1:
...
def per_step_metrics(pred: torch.Tensor, target: torch.Tensor) -> dict:
    ...
    rel_l2 = (pred.float() - target.float()).norm(dim=-1) / target.float().norm(dim=-1).clamp_min(1e-8)
    return {
        "cos": cos.cpu().numpy(),
        "mse": mse.cpu().numpy(),
...
def main() -> None:
    ...
    for i, (raw_p, hid_p, demo_key) in enumerate(pairs):
        ...
        print(f"  [{i+1}/{len(pairs)}] {Path(raw_p).stem}/{demo_key} T={T} "
    ...
    def aggregate(per_demo: list[dict]) -> dict:
        ...
        return {
            "cos_mean": cos.mean(0), "cos_std": cos.std(0),
            "mse_mean": mse.mean(0), "mse_std": mse.std(0),
            "rel_l2_mean": rl2.mean(0), "rel_l2_std": rl2.std(0),
            "T_pred": int(min_T),
    ...
    print(f"{'chunk':>5} {'env-step':>8}   {'open cos':>10}  {'close cos':>10}   "
    ...
    for c in range(0, T_show // K):
        ...
        print(f"{c:>5} {env_step:>8}   {o_cos:>10.4f}  {c_cos:>10.4f}   "
              f"{o_mse:>10.4f}  {c_mse:>10.4f}   {o_rl2:>10.4f}  {c_rl2:>10.4f}")

    print("\n=== aggregate over all predicted env-steps ===")
    print(f"  open : cos {open_agg['cos_mean'].mean():.4f}  mse {open_agg['mse_mean'].mean():.4f}  "
          f"rel_l2 {open_agg['rel_l2_mean'].mean():.4f}  ({open_agg['n_demos']} demos x {open_agg['T_pred']} steps)")
    print(f"  close: cos {close_agg['cos_mean'].mean():.4f}  mse {close_agg['mse_mean'].mean():.4f}  "
    ...
    if args.out:
        ...
        Path(args.out).write_text(json.dumps({
            ...
            "open":  {k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in open_agg.items()},
            "close": {k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in close_agg.items()},
        }))
