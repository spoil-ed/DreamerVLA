...
`download/20_libero_dataset.sh` is the default LIBERO data route. It downloads
the pinned combined `physical-intelligence/libero` LeRobot snapshot once through
`https://hf-mirror.com` with proxy variables cleared. It does not provide raw
suite-by-suite HDF5; those files belong only to explicit OpenVLA preprocessing.
