...
USE_PRESEEDED_THIRD_PARTY="${USE_PRESEEDED_THIRD_PARTY:-0}"
...
echo "[install:40_third_party] use_preseeded_third_party=${USE_PRESEEDED_THIRD_PARTY}"
mkdir -p "${DVLA_ROOT}/third_party"

if [[ ! -d "${DVLA_ROOT}/third_party/LIBERO/.git" ]]; then
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/LIBERO checkout." >&2; exit 2; fi
  git clone https://github.com/Lifelong-Robot-Learning/LIBERO.git "${DVLA_ROOT}/third_party/LIBERO"
fi
if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/LIBERO" fetch --all --tags; fi
git -C "${DVLA_ROOT}/third_party/LIBERO" checkout 8f1084e

if [[ ! -d "${DVLA_ROOT}/third_party/robosuite/.git" ]]; then
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/robosuite checkout." >&2; exit 2; fi
  git clone https://github.com/ARISE-Initiative/robosuite.git "${DVLA_ROOT}/third_party/robosuite"
fi
if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/robosuite" fetch --all --tags; fi
git -C "${DVLA_ROOT}/third_party/robosuite" checkout b9d8d3de5e3dfd1724f4a0e6555246c460407daa

if [[ ! -d "${DVLA_ROOT}/third_party/robosuite-task-zoo/.git" ]]; then
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/robosuite-task-zoo checkout." >&2; exit 2; fi
  git clone https://github.com/ARISE-Initiative/robosuite-task-zoo "${DVLA_ROOT}/third_party/robosuite-task-zoo"
fi
if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/robosuite-task-zoo" fetch --all --tags; fi
git -C "${DVLA_ROOT}/third_party/robosuite-task-zoo" checkout 74eab7f88214c21ca1ae8617c2b2f8d19718a9ed

if [[ ! -d "${DVLA_ROOT}/third_party/robomimic/.git" ]]; then
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/robomimic checkout." >&2; exit 2; fi
  git clone https://github.com/ARISE-Initiative/robomimic.git "${DVLA_ROOT}/third_party/robomimic"
fi
if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/robomimic" fetch --all --tags; fi
git -C "${DVLA_ROOT}/third_party/robomimic" checkout d0b37cf214bd24fb590d182edb6384333f67b661

if [[ ! -d "${DVLA_ROOT}/third_party/mimicgen/.git" ]]; then
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/mimicgen checkout." >&2; exit 2; fi
  git clone https://github.com/NVlabs/mimicgen.git "${DVLA_ROOT}/third_party/mimicgen"
fi
if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/mimicgen" fetch --all --tags; fi
...
# mimicgen leaves chardet unbounded, while requests 2.32.3 warns for chardet
# 6 or newer. Keep the optional detector inside requests' supported range.
uv pip install chardet==5.2.0
...
if [[ "${INSTALL_OPENSORA_THIRD_PARTY}" == "1" ]]; then
  if [[ ! -e "${DVLA_ROOT}/third_party/opensora" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/opensora checkout." >&2; exit 2; fi
  ...
  if [[ -d "${DVLA_ROOT}/third_party/opensora/.git" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/opensora" fetch --all --tags; fi
...
if [[ "${INSTALL_OPENVLA_OFT_THIRD_PARTY}" == "1" ]]; then
  if [[ ! -e "${DVLA_ROOT}/third_party/openvla-oft" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/openvla-oft checkout." >&2; exit 2; fi
  ...
  if [[ -d "${DVLA_ROOT}/third_party/openvla-oft/.git" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/openvla-oft" fetch --all --tags; fi
