...
echo "[install:50_special_packages] use_preseeded_third_party=${USE_PRESEEDED_THIRD_PARTY}"
...
if [[ "${INSTALL_FLASH_ATTN}" == "1" ]]; then
  ...
  if [[ ! -f "${FLASH_ATTN_WHEEL}" ]]; then
    FLASH_ATTN_PARTIAL="${FLASH_ATTN_WHEEL}.partial"
    curl --fail --location --retry 5 --retry-delay 2 --continue-at - \
      "${FLASH_ATTN_WHEEL_URL}" -o "${FLASH_ATTN_PARTIAL}"
    mv "${FLASH_ATTN_PARTIAL}" "${FLASH_ATTN_WHEEL}"
  fi
  uv pip install "${FLASH_ATTN_WHEEL}"
fi

if [[ "${INSTALL_EGL_PROBE}" == "1" ]]; then
  if [[ ! -d "${DVLA_ROOT}/third_party/egl_probe/.git" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/egl_probe checkout." >&2; exit 2; fi
    git clone https://github.com/StanfordVL/egl_probe.git "${DVLA_ROOT}/third_party/egl_probe"
  fi
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/egl_probe" fetch --all --tags; fi
...
if [[ "${INSTALL_OPTIONAL_THIRD_PARTY}" == "1" ]]; then
  if [[ ! -d "${DVLA_ROOT}/third_party/apex/.git" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/apex checkout." >&2; exit 2; fi
    git clone https://github.com/NVIDIA/apex.git "${DVLA_ROOT}/third_party/apex"
  fi
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/apex" fetch --all --tags; fi
  git -C "${DVLA_ROOT}/third_party/apex" checkout 5daec2a
  if [[ ! -d "${DVLA_ROOT}/third_party/TensorNVMe/.git" ]]; then
    if [[ "${USE_PRESEEDED_THIRD_PARTY}" == "1" ]]; then echo "Missing preseeded third_party/TensorNVMe checkout." >&2; exit 2; fi
    git clone https://github.com/hpcaitech/TensorNVMe.git "${DVLA_ROOT}/third_party/TensorNVMe"
  fi
  if [[ "${USE_PRESEEDED_THIRD_PARTY}" != "1" ]]; then git -C "${DVLA_ROOT}/third_party/TensorNVMe" fetch --all --tags; fi
