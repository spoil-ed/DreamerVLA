...
if [[ "${INSTALL_DEV_TOOLS}" == "1" && "${INSTALL_RAY}" == "1" ]]; then
  echo "[install:30_python_deps] dev_dependency_group=dev ray_extra=ray"
  uv pip install --editable "${DVLA_ROOT}[ray]" --group dev
elif [[ "${INSTALL_DEV_TOOLS}" == "1" ]]; then
  echo "[install:30_python_deps] dev_dependency_group=dev"
  uv pip install --group dev
elif [[ "${INSTALL_RAY}" == "1" ]]; then
  echo "[install:30_python_deps] dev_dependency_group=skipped ray_extra=ray"
  uv pip install --editable "${DVLA_ROOT}[ray]"
else
  echo "[install:30_python_deps] dev_dependency_group=skipped ray_extra=skipped"
fi
