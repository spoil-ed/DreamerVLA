...
Use ``dreamervla.dataset.base.latent_token_dataloader.find_demo_pairs`` to
...
from dreamervla.dataset.base.latent_token_dataloader import (
    ...
    *,
    ...
    for raw_p, hid_p, demo_key in pairs:
        ...
        if rec is None or rec.finish_step < min_T:
            ...
            continue
        out.append(rec)
    logger.info(f"[lumos-latent:{label}] loaded {len(out)} demos (skipped {skipped})")
    return out
...
class LumosAlignedLatentTrainDataset(IterableDataset):
    ...
    def __init__(
        ...
        succ_pairs = _partition_demo_pairs(
            find_demo_pairs(success_dir_raw, success_dir_hidden),
        ...
        if failure_dir_raw is not None and failure_dir_hidden is not None:
            fail_pairs = _partition_demo_pairs(
                find_demo_pairs(failure_dir_raw, failure_dir_hidden),
                split=self.demo_split,
                val_fraction=self.val_fraction,
                split_seed=self.split_seed,
            )
...
class LumosAlignedLatentValDataset(Dataset):
    ...
    def __init__(
        ...
        if verbose:
            ...
            logger.info(f"[lumos-latent:val] total windows={len(slots)}  pos={n_pos}  neg={n_neg}")
