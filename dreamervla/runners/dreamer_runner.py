...
class DreamerRunner(BaseRunner):
    ...
    def __init__(self, cfg: dict[str, Any] | DictConfig) -> None:
        ...
        disabled = [
            ...
            if bool(OmegaConf.select(config, key, default=False)) is not expected
        ]
        if disabled:
            raise ValueError(
                f"DreamerRunner mode {training_mode!r} has invalid settings: {disabled}"
            )
    ...
    def run(self) -> dict[str, float | int]:
        ...
        try:
            ...
            _hs_trace("[cotrain] resume payload load start")
            ...
            print(
                "[cotrain] groups=" + ",".join(self._target_group_names()),
                flush=True,
            )
    ...
    def _build_groups(self, cluster: Cluster) -> dict[str, Any]:
        ...
        eval_env_channel_name = f"cotrain-eval-env-{uuid.uuid4().hex}"
        eval_rollout_channel_name = f"cotrain-eval-rollout-{uuid.uuid4().hex}"
        eval_actor_channel_name = f"cotrain-eval-actor-{uuid.uuid4().hex}"
        ...
        eval_env_channel = Channel.create(eval_env_channel_name)
        eval_rollout_channel = Channel.create(eval_rollout_channel_name)
        eval_actor_channel = Channel.create(eval_actor_channel_name)
        replay_group = None
        replay = None
        ...
        real_env_group = None
        if not real_specs:
            raise ValueError("DreamerRunner placement requires a real_env worker")
        ...
        if wm_specs:
            ...
            raise ValueError("DreamerRunner requires at least one WMEnvWorker")

        evaluation_env_group = None
        ...
        learner_group = None
        learner_progress_path: Path | None = None
        if plan.learner_spec is None:
            raise ValueError("DreamerRunner requires a learner placement")
        _hs_trace("[build_groups] launch LearnerWorker start")
        learner_train_cfg = self._cfg_dict("learner.train_cfg")
        learner_progress_path = self.get_diagnostics_dir() / "cotrain_learner_progress.json"
        learner_train_cfg["progress_path"] = str(learner_progress_path)
        ...
        return {
            ...
            "learner_progress_path": learner_progress_path,
            ...
            "EvaluationEnvGroup": evaluation_env_group,
            ...
            "eval_env_channel": eval_env_channel,
            "eval_rollout_channel": eval_rollout_channel,
            "eval_actor_channel": eval_actor_channel,
            "eval_env_channel_name": eval_env_channel_name,
    ...
    def _learner_init_checkpoint(self) -> dict[str, Any]:
        """Resolve a consolidated checkpoint or independent trainable WM/CLS states."""
        ...
        loaded_classifier = load_component_checkpoint(classifier_path, "classifier")
    ...
    def _run_staged_global_step(
        ...
        imagined_progress_monitor = _ManualCotrainEnvProgressMonitor(
            progress_dir,
            self.console_progress,
            desc=f"cotrain-imagined-rollout/{int(global_step):08d}",
            roles={"wm_env"},
        )
        real_progress_monitor.report(force=True)
        if self._wm_rollout_target_trajectories() is None:
            imagined_progress_monitor.report(force=True)
        ...
        self._report_phase_completion(
            "cotrain-real-rollout",
            global_step,
            done=real_batch.num_trajectories,
            total=(expected_real or real_batch.num_trajectories),
            unit="trajectory",
            status=f"successes={real_batch.num_successes}",
        )
        ...
        if dynamic_wm_leases:
            ...
            wm_env_metrics = self._wait_env_metrics_with_dynamic_wm_leases(
                ...
                actor_channel_name=actor_channel_name,
                timeout_s=self._env_rollout_timeout_s(),
                progress=imagined_progress_monitor,
            )
        else:
        ...
        imagined_done = int(
            float(
                wm_env_metrics.get(
                    "env/wm_env/trajectories_completed",
                    wm_env_metrics.get("env/wm_env/trajectory_shards", imagined_total or 0),
                )
            )
        )
        self._report_phase_completion(
    ...
    def _report_phase_completion(
        ...
        *,
        ...
        self.console_progress(
            min(max(0, int(done)), resolved_total),
            resolved_total,
            f"{name}/{int(global_step):08d}",
            unit=unit,
            status=(status or None),
            force=True,
        )

    def _wait_learner_update_with_progress(
        self,
        result: Any,
        *,
        ...
        """Wait for the resident learner while streaming its step-local progress."""

        if progress_path is None or not callable(getattr(result, "done", None)):
            return result.wait()
        ...
        interval = max(
            ...
            min(
                1.0,
                float(OmegaConf.select(self.cfg, "console.progress_every_s", default=5.0)),
            ),
        )
        last_done = -1
        while not result.done():
            payload = _read_json_mapping(path)
            done = int(payload.get("train_step", 0)) if payload else 0
            current_total = int(payload.get("total_train_steps", total)) if payload else total
            if done != last_done:
                self.console_progress(
                    done,
                    max(1, current_total),
                    f"cotrain-wmcls-training/{int(global_step):08d}",
                    unit="update",
                    status=_learner_progress_status(payload),
                    force=True,
                )
                last_done = done
            time.sleep(interval)
        values = result.wait()
        payload = _read_json_mapping(path)
        done = int(payload.get("train_step", total)) if payload else total
        current_total = int(payload.get("total_train_steps", total)) if payload else total
        self.console_progress(
            done,
            max(1, current_total),
    ...
    def _maybe_evaluate_resident_policy(
        self,
        groups: dict[str, Any],
        global_step: int,
    ) -> dict[str, float]:
        ...
        if interval <= 0 or int(global_step) % interval != 0:
            return {}
        return self._evaluate_resident_policy(
            groups,
            global_step=int(global_step),
            sync_policy=True,
        )

    def _evaluate_resident_policy(
        self,
        groups: dict[str, Any],
        *,
        global_step: int,
        ...
        evaluation_env = groups.get("EvaluationEnvGroup")
        if evaluation_env is None:
            raise RuntimeError(
                "resident evaluation is enabled but EvaluationEnvGroup was not launched"
            )
        actor = groups["ActorGroup"]
        rollout = groups["RolloutGroup"]
        ...
        progress = _EvaluationProgressMonitor(
            progress_dir,
            self.console_progress,
            desc=f"eval/{int(global_step):08d}",
            total_episodes=self._eval_num_episodes(),
        )
        progress.report(force=True)
        sync_metrics: dict[str, float] = {}
        if sync_policy:
            raw_sync = self._sync_policy_groups(
                actor,
                rollout,
                version=self._eval_policy_sync_version(global_step),
            )
            sync_metrics = {
                f"eval_sync/{key.removeprefix('sync/')}": float(value)
                for key, value in raw_sync.items()
            }

        started = time.perf_counter()
        env_result = evaluation_env.interact(
            str(groups["eval_env_channel_name"]),
            str(groups["eval_rollout_channel_name"]),
            str(groups["eval_actor_channel_name"]),
        )
        ...
        successes = min(
            ...
            max(
                ...
                int(
                    float(
                        env_metrics.get(
                            "env/eval_env/episodes_successful",
                            env_metrics.get("env/episodes_successful", 0.0),
                        )
                    )
                ),
            ),
        ...
        chunk_steps = float(
            env_metrics.get(
                ...
                env_metrics.get("env/chunk_steps", 0.0),
            )
        )
        self.console_progress(
            episodes,
            episodes,
            f"eval/{int(global_step):08d}",
            unit="episode",
            status=(
    ...
    def _evaluation_enabled(self) -> bool:
        return self._eval_interval_global_steps() > 0 or self._eval_initial_global_step()

    def _eval_initial_global_step(self) -> bool:
        return bool(
            OmegaConf.select(
                self.cfg,
    ...
    def _eval_task_ids(self) -> tuple[int, ...]:
        value = OmegaConf.select(
            self.cfg,
            "manual_cotrain.eval_protocol.task_ids",
            default=self._real_task_ids(),
        )
        ...
        if not task_ids or len(task_ids) != len(set(task_ids)):
            raise ValueError("manual_cotrain.eval_protocol.task_ids must be non-empty and unique")
        return task_ids

    def _eval_num_envs(self) -> int:
        value = int(
            OmegaConf.select(
                self.cfg,
                "manual_cotrain.eval_protocol.num_envs",
                default=25,
            )
        )
        if value <= 0:
            raise ValueError("manual_cotrain.eval_protocol.num_envs must be positive")
        return value

    def _eval_num_episodes(self) -> int:
        per_task = int(
            OmegaConf.select(
                self.cfg,
                "manual_cotrain.eval_protocol.num_episodes_per_task",
                default=10,
            )
        )
        if per_task <= 0:
    ...
    def _eval_max_steps(self) -> int:
        requested = int(
            OmegaConf.select(
                self.cfg,
                "manual_cotrain.eval_protocol.max_steps",
                default=300,
            )
        )
        if requested <= 0:
    ...
    def _staged_policy_sync_version(global_step: int, *, post_sft: bool) -> int:
        # Three causally distinct policy snapshots are reserved per step:
        # step-entry, post-SFT imagination, and post-PPO evaluation.
        return 3 * int(global_step) - (1 if post_sft else 2)

    @staticmethod
    def _eval_policy_sync_version(global_step: int) -> int:
        return 3 * int(global_step)
    ...
    def _maybe_save_manual_checkpoint(
        ...
        *,
        ...
        if learner_group is None:
            raise RuntimeError("DreamerRunner checkpoint requires LearnerGroup")
...
class _ManualCotrainEnvProgressMonitor:
    def __init__(
        ...
        *,
        ...
        self._desc = str(desc)
        self._roles = None if roles is None else {str(role) for role in roles}

    def report(self, *, force: bool = False) -> _ManualCotrainProgressSnapshot:
        snapshot = _read_manual_cotrain_progress_snapshot(
            self.progress_dir,
            roles=self._roles,
        )
        if snapshot.total > 0:
...
def _learner_progress_status(payload: dict[str, Any]) -> str | None:
    if not payload:
        return None
    parts = [f"phase={payload.get('phase', 'starting')}"]
    wm_total = int(payload.get("wm_total_steps", 0))
    cls_total = int(payload.get("cls_total_steps", 0))
    if wm_total:
        parts.append(f"wm={int(payload.get('wm_step', 0))}/{wm_total}")
    if cls_total:
        parts.append(f"cls={int(payload.get('cls_step', 0))}/{cls_total}")
    return " ".join(parts)
