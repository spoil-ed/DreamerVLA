...
def _source_tree_sha256(root: pathlib.Path) -> str:
    """Hash the submitted source tree when Git metadata is unavailable."""
    ...
    paths = sorted(
        ...
        if path.is_file()
        and not any(part in _SOURCE_HASH_SKIP_PARTS for part in path.relative_to(root).parts)
    )
    for path in paths:
...
@functools.lru_cache(maxsize=1)
def _git_worktree_sha256(root: pathlib.Path, commit: str) -> str:
    """Hash one Git commit plus its tracked diff and untracked source files."""
    ...
    for relative_text in sorted(untracked_output.splitlines()):
        relative = pathlib.Path(relative_text)
        if any(part in _SOURCE_HASH_SKIP_PARTS for part in relative.parts):
            continue
        path = root / relative
        if not path.is_file():
            continue
...
class BaseRunner(ABC):
    ...
    def get_resolved_config_path(self) -> pathlib.Path:
        """Return the native-Hydra location for the resolved runtime config."""

        return self.get_artifact_dir(".hydra", "resolved_config.yaml")
    ...
    def write_run_artifacts(self) -> dict[str, Any] | None:
        ...
        resolved_config = OmegaConf.to_yaml(self.cfg, resolve=True, sort_keys=True)
        resolved_config_path = self.get_resolved_config_path()
    ...
    def build_run_manifest(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config": {
                "path": ".hydra/resolved_config.yaml",
                "sha256": hashlib.sha256(self.get_resolved_config_path().read_bytes()).hexdigest(),
            },
            "git": git_metadata,
            "source": self._source_metadata(git_metadata),
        }
    ...
    def _git_metadata() -> dict[str, Any]:
        ...
        commit = capture("rev-parse", "HEAD") or os.environ.get("DVLA_GIT_COMMIT")
        branch = capture("rev-parse", "--abbrev-ref", "HEAD") or os.environ.get("DVLA_GIT_BRANCH")
        dirty: bool | None = None if status is None else bool(status)
        if dirty is None and "DVLA_GIT_DIRTY" in os.environ:
            dirty = os.environ["DVLA_GIT_DIRTY"].strip().lower() in {
                "1",
                "true",
                "yes",
            }
    ...
    @staticmethod
    def _source_metadata(git_metadata: Mapping[str, Any]) -> dict[str, Any]:
        """Record exact source identity for Git checkouts and submitted archives."""

        git_commit = git_metadata.get("commit")
        git_dirty = git_metadata.get("is_dirty")
        configured_hash = os.environ.get("DVLA_SOURCE_SHA256")
        tree_hash = configured_hash
        if tree_hash is None and git_commit is not None and git_dirty is True:
            tree_hash = _git_worktree_sha256(PROJECT_ROOT, str(git_commit))
        elif tree_hash is None and git_commit is None:
            tree_hash = _source_tree_sha256(PROJECT_ROOT)
        return {
            "kind": "git" if git_commit is not None else "archive",
            "tree_sha256": tree_hash,
            "sinfra_job_id": os.environ.get("SINFRA_JOB_ID"),
            "image_digest": os.environ.get("DVLA_IMAGE_DIGEST"),
        }
