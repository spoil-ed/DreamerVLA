...
lives on ``LIBEROVLAEvaluationBase.evaluate_libero`` so there is exactly one
...
class LIBEROVLAEvaluationRunner(
    ...
    def run(self) -> list[dict[str, Any]]:
        ...
        # ── optional: load VLA checkpoint (produced by LIBEROVLAEvaluationBase) ─
        if ckpt_path and not ckpt_is_hf_vla:
            ...
            # (The ckpt was produced by LIBEROVLAEvaluationBase which writes
    ...
    def _normalize_manual_ray_dreamer_eval_cfg(cls, cfg: DictConfig) -> None:
        """Adapt manual Ray checkpoint schema to LIBEROVLAEvaluationRunner's schema."""
    ...
    def _load_module_state(self, module: Any, state_dict: dict[str, Any], name: str) -> None:
        ...
        preserve_parameter_dtypes = bool(getattr(module, "preserve_parameter_dtypes", False))
        converted = {
