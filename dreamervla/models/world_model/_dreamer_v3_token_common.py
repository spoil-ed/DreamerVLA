...
class _DreamerV3TokenWorldModelBase(DreamerV3ActorAdapterMixin):
    """Shared RSSM/encoder/decoder + token CE loss for DreamerV3 token WMs.

    Subclasses supply only the class name/docstring and ``_dones_supports_is_terminal``
    (whether ``loss`` falls back to ``batch["is_terminal"]`` for the continue target).
    """

    # When True, ``loss`` reads the continue target from ``is_terminal`` if present,
    # otherwise ``dones``. When False, it always reads ``dones``.
    _dones_supports_is_terminal: bool = False
    ...
    def loss(self, batch: dict[str, torch.Tensor]) -> DreamerV3Loss:
        ...
        if self._dones_supports_is_terminal:
