...
class BasePolicy(ABC):
    ...
    def enable_torch_compile(self, mode: str = "max-autotune-no-cudagraphs"):
        raise NotImplementedError(
            "torch compile is not supported for current policy, please set "
            "`enable_torch_compile=False` for now"
        )

    def capture_cuda_graph(self, train_batch_size: int, eval_batch_size: int):
        raise NotImplementedError(
            "cuda graph is not supported for current policy, please set "
            "`enable_cuda_graph=False` for now"
        )
