...
from dreamervla.runtime.rollout.vectorized_collect import build_step_record, proprio_from_record
...
def build_dump_step(
    *,
    ...
    step = build_step_record(
        full_record,
        (np.asarray(obs_embedding, dtype=np.float16) if obs_embedding is not None else None),
        action,
        lang_emb=lang_emb,
    )
