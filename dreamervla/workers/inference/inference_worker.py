...
class InferenceWorker(Worker):
    ...
    def __init__(self, model_cfg: dict[str, Any], init_ckpt: dict[str, Any], num_envs: int) -> None:
        ...
        self.torch_device = torch.device(str(self.model_cfg.get("device", self.device)))
    ...
    @torch.no_grad()
    def forward_batch(self, obs_batch: list[dict[str, Any]], env_ids: list[int]) -> dict[str, list[Any]]:
        ...
        actions = action_chunk.reshape(action_chunk.shape[0], -1, action_chunk.shape[-1])[:, 0, :7]
        ...
        return {
            ...
            "obs_embedding": [
                row.detach().cpu().numpy().astype(np.float32) for row in obs_embedding
            ],
        }
    ...
    def _load_initial_weights(self) -> None:
        ...
        if "policy" in self.init_ckpt:
            self._policy().load_state_dict(_to_device_state(self.init_ckpt["policy"], self.torch_device))
