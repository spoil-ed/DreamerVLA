...
def _auto_apply_distributed(cfg: DictConfig) -> None:
    """Enable DDP under torchrun unless the recipe explicitly selects FSDP."""
    ...
    if training is None:
        return
    configured_strategy = str(training.get("distributed_strategy", "")).lower()
    if "distributed_strategy" in training and configured_strategy not in {
        "fsdp",
        "fsdp1",
        "fsdp2",
    }:
