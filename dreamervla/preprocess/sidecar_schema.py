...
PREFIX_INPUT_LATENT_SCHEMA_VERSION = 3
...
def _pi05_prefix_input_contract_errors(config: Mapping[str, Any]) -> list[str]:
    errors = _observation_latent_geometry_errors(config)
    expected = {
        "policy_family": "pi05",
        "obs_hidden_source": PI05_PREFIX_INPUT_SOURCE,
        "action_head_type": PI05_ACTION_HEAD,
        "sidecar_schema_version": PREFIX_INPUT_LATENT_SCHEMA_VERSION,
        "history": 1,
        "include_state": True,
    ...
    for key, wanted in expected.items():
        ...
        if got != wanted:
            errors.append(f"{key}={got!r}, expected {wanted!r}")
    if config.get("token_count") != 968:
        errors.append(f"token_count={config.get('token_count')!r}, expected 968")
    if config.get("token_dim") != 2048:
        errors.append(f"token_dim={config.get('token_dim')!r}, expected 2048")
    image_tokens = config.get("image_token_count")
    text_tokens = config.get("text_token_count")
    if type(image_tokens) is int and type(text_tokens) is int:
        if image_tokens + text_tokens != config.get("token_count"):
            errors.append(
                "image_token_count + text_token_count must equal token_count: "
                f"{image_tokens} + {text_tokens} != {config.get('token_count')!r}"
            )
    if config.get("text_mode") not in {"exact", "masked"}:
        errors.append(f"text_mode={config.get('text_mode')!r}, expected exact or masked")
    for key in ("wm_use_text", "policy_use_text"):
        if type(config.get(key)) is not bool:
            errors.append(f"{key}={config.get(key)!r}, expected a boolean")
    return errors
...
def validate_hidden_token_preprocess_config(
    ...
    *,
    ...
    if source == PI05_PREFIX_SOURCE:
        errors = _pi05_prefix_contract_errors(config)
    elif source == PI05_PREFIX_INPUT_SOURCE:
        errors = _pi05_prefix_input_contract_errors(config)
    else:
