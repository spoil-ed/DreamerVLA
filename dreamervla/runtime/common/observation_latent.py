...
from dreamervla.preprocess.sidecar_schema import (
    ...
    PREFIX_INPUT_LATENT_SCHEMA_VERSION,
...
class ObservationLatentSpec:
    ...
    wm_use_text: bool | None = None
    policy_use_text: bool | None = None
    predict_text_tokens: bool | None = None
    prefix_attention_mask_key: str | None = None
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> ObservationLatentSpec:
        ...
        return cls(
            ...
            image_token_count=(
                int(value["image_token_count"])
                if value.get("image_token_count") is not None
                else None
            ),
            text_token_count=(
                int(value["text_token_count"])
                if value.get("text_token_count") is not None
                else None
            ),
            text_mode=(str(value["text_mode"]) if value.get("text_mode") is not None else None),
            wm_use_text=(
                bool(value["wm_use_text"]) if value.get("wm_use_text") is not None else None
            ),
            policy_use_text=(
                bool(value["policy_use_text"]) if value.get("policy_use_text") is not None else None
            ),
            predict_text_tokens=(
                bool(value["predict_text_tokens"])
                if value.get("predict_text_tokens") is not None
                else None
            ),
            prefix_attention_mask_key=(
                str(value["prefix_attention_mask_key"])
                if value.get("prefix_attention_mask_key") is not None
                else None
            ),
        )
        ...
        config: dict[str, Any] = {
            **asdict(self),
            ...
            "sidecar_schema_version": (
                PREFIX_INPUT_LATENT_SCHEMA_VERSION
                if self.obs_hidden_source == PI05_PREFIX_INPUT_SOURCE
                else OBSERVATION_LATENT_SCHEMA_VERSION
            ),
