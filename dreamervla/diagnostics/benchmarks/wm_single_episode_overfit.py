...
from dreamervla.utils.checkpoint.run_artifacts import load_run_config
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Required input: cotrain run config containing ray_components.world_model/classifier.",
    )
    parser.add_argument(
        "--resolved-config",
        dest="run_config",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Required for check/train/all: checkpoint with a top-level world_model state dict.",
    )
    parser.add_argument(
        "--trained-wm-ckpt",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Required input: classifier checkpoint containing state_dicts.classifier.",
    )
    parser.add_argument(
        "--hidden-hdf5",
        type=Path,
...
def _require_path(path: Path | None, label: str) -> None:
    if path is None:
        raise ValueError(f"{label} is required; supply its command-line path")
    if not path.exists():
...
def _required_paths(args: argparse.Namespace) -> dict[str, Path | None]:
    paths = {
...
def main() -> None:
    ...
    plan = {
        ...
        "stage": args.stage,
        "run_config": str(args.run_config) if args.run_config is not None else None,
        "wm_ckpt": str(args.wm_ckpt) if args.wm_ckpt is not None else None,
        "trained_wm_ckpt": str(args.trained_wm_ckpt),
        "classifier_ckpt": str(args.classifier_ckpt) if args.classifier_ckpt is not None else None,
