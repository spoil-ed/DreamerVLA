...
from dreamervla.utils.logging.json_logger import JsonLogger
...
class NopretokenizeSFTDistributedHelper:
    ...
    def object_barrier(self) -> None:
        """Synchronize control flow on the CPU object-collective group.

        Standalone evaluation does not exchange CUDA tensors between ranks.  It
        uses barriers only to coordinate progress files and metric gathering,
        so routing those barriers through the existing Gloo group avoids
        needlessly initializing an NCCL communicator in inference-only jobs.
        """
