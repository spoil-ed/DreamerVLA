...
from dreamervla.utils.logging.metrics import coerce_metric_scalar
...
class MetricLogger:
    ...
    def __init__(
        ...
        *,
        ...
        self.logger_backends = normalize_logger_backends(logger_backends)
        unsupported = [
...
def normalize_logger_backends(value: Any) -> list[str]:
    """Normalize configured backends and discard explicit disabled values."""
    if value is None:
        return []
...
def _normalize_backend_filter(
    ...
    backends = normalize_logger_backends(value)
    if not backends:
...
__all__ = ["MetricLogger", "NullMetricLogger", "normalize_logger_backends"]
