...
def _validate_vla_sft_contract(cfg: DictConfig, *, world_size: int) -> None:
    ...
    if loader_batch != micro:
        raise ValueError("VLA SFT data.loader.batch_size must match actor.micro_batch_size")
    ...
    _validate_pi05_training_data_pipeline(cfg, route="LIBERO SFT")
    ...
    actor_strategy = str(OmegaConf.select(cfg, "actor.distributed.strategy", default="")).lower()
    training_strategy = str(
        OmegaConf.select(cfg, "training.distributed_strategy", default="")
    ).lower()
    if actor_strategy not in {"ddp", "fsdp"}:
        raise ValueError("π0.5 SFT actor.distributed.strategy must be ddp or fsdp")
    if training_strategy != actor_strategy:
        raise ValueError(
            "π0.5 SFT training.distributed_strategy must match actor.distributed.strategy"
        )
    ...
    policy_batch = int(OmegaConf.select(cfg, "actor.policy_cfg.kwargs.batch_size", default=0) or 0)
    if policy_batch != global_batch:
        raise ValueError("π0.5 policy batch_size must match actor.global_batch_size")
    policy_steps = int(
        OmegaConf.select(
            cfg,
            "actor.policy_cfg.kwargs.total_training_steps",
            default=0,
        )
        or 0
    )
    optimizer_steps = int(OmegaConf.select(cfg, "actor.optim.total_training_steps", default=0) or 0)
    if policy_steps != max_steps or optimizer_steps != max_steps:
        raise ValueError("π0.5 policy, optimizer, and training total steps must match")
    milestones = [
        int(step) for step in (OmegaConf.select(cfg, "training.milestone_steps", default=[]) or [])
    ]
    if milestones != sorted(set(milestones)):
        raise ValueError("VLA SFT milestone_steps must be sorted and unique")
    if any(step <= 0 or step > max_steps for step in milestones):
        raise ValueError("VLA SFT milestone_steps must be within training.max_steps")
    if actor_strategy == "ddp" and bool(
        OmegaConf.select(
            cfg,
            "actor.distributed.find_unused_parameters",
            default=False,
        )
    ):
        raise ValueError(
            "π0.5 DDP freezes its unused lm_head and requires find_unused_parameters=false"
        )
...
def _validate_pi05_training_data_pipeline(cfg: DictConfig, *, route: str) -> None:
    ...
    if str(OmegaConf.select(cfg, "data.loader.repo_id", default="")) != (
        ...
        raise ValueError(f"π0.5 {route} requires the physical-intelligence/libero dataset")
    if str(OmegaConf.select(cfg, "data.loader.revision", default="")) != (
        "a4336d589d589045d1c56423ffdf3b88a0e19b1f"
    ):
        raise ValueError(f"π0.5 {route} requires the pinned official dataset revision")
    loader_config_name = str(OmegaConf.select(cfg, "data.loader.config_name", default=""))
    task_config_name = str(OmegaConf.select(cfg, "task.pi05.config_name", default=""))
    if loader_config_name != task_config_name:
        raise ValueError(f"π0.5 {route} data.loader.config_name must match task.pi05.config_name")
    if not bool(OmegaConf.select(cfg, "data.noops_filtered", default=False)):
        raise ValueError(f"π0.5 {route} requires the no-op-filtered LeRobot snapshot")
...
def _validate_pi05_contract(cfg: DictConfig) -> None:
    ...
    _validate_pi05_prefix_input_contract(cfg)


def _validate_pi05_prefix_input_contract(cfg: DictConfig) -> None:
    """Validate the native pre-PaliGemma `[768 image + 200 text, 2048]` route."""

    key = "task.prefix_input_latent"
    ...
    image_tokens = _select_int(cfg, f"{key}.image_token_count")
    text_tokens = _select_int(cfg, f"{key}.text_token_count")
    assert image_tokens is not None and text_tokens is not None and token_count is not None
    if image_tokens + text_tokens != token_count:
        raise ValueError(
            "task.prefix_input_latent requires image_token_count + text_token_count "
            f"== token_count ({image_tokens} + {text_tokens} != {token_count})"
        )
    if image_tokens != 768 or text_tokens != 200 or token_count != 968 or token_dim != 2048:
        raise ValueError(
            "task.prefix_input_latent must use image_token_count=768, "
            "text_token_count=200, token_count=968, and token_dim=2048"
        )
    if _select_str(cfg, f"{key}.obs_hidden_source") != "prefix_input_latent":
        raise ValueError("task.prefix_input_latent.obs_hidden_source must be prefix_input_latent")
    text_mode = _select_str(cfg, f"{key}.text_mode")
    if text_mode not in {"exact", "masked"}:
        raise ValueError("task.prefix_input_latent.text_mode must be exact or masked")
    for field in ("wm_use_text", "policy_use_text"):
        value = OmegaConf.select(cfg, f"{key}.{field}", default=None)
        if type(value) is not bool:
            raise ValueError(f"task.prefix_input_latent.{field} must be a boolean")
    if bool(OmegaConf.select(cfg, f"{key}.predict_text_tokens", default=True)):
        raise ValueError("task.prefix_input_latent.predict_text_tokens must be false")
    ...
    for component_key in (
        ...
        target = _component_target(cfg, component_key)
        if target is None or not target.endswith("Pi05PrefixInputWorldModel"):
            continue
        for field, expected in (
            ...
            actual = _select_int(cfg, f"{component_key}.{field}")
            if actual != expected:
                raise ValueError(
                    f"{component_key}.{field} must match task.prefix_input_latent "
                    f"({actual} != {expected})"
                )
        if _select_str(cfg, f"{component_key}.text_mode") != text_mode:
            raise ValueError(f"{component_key}.text_mode must match task.prefix_input_latent")
        for field in ("wm_use_text", "policy_use_text"):
            actual = OmegaConf.select(cfg, f"{component_key}.{field}", default=None)
            expected = OmegaConf.select(cfg, f"{key}.{field}")
            if type(actual) is not bool or actual != expected:
                raise ValueError(
                    f"{component_key}.{field} must match task.prefix_input_latent "
                    f"({actual!r} != {expected!r})"
                )
        if bool(OmegaConf.select(cfg, f"{component_key}.predict_text_tokens", default=True)):
            raise ValueError(f"{component_key}.predict_text_tokens must be false")
        normalization = _select_str(cfg, f"{component_key}.token_normalization")
        if normalization is not None and normalization != "none":
            raise ValueError(
                f"{component_key}.token_normalization must be none for native prefix inputs"
            )
...
def _validate_chunk_wm_sequence_length_for_component(
    ...
    if not _is_chunk_world_model_target(target):
        return
...
def _validate_latent_dimension_contracts(cfg: DictConfig) -> None:
    for key in ("task.openvla_oft.hidden_token", "task.prefix_input_latent"):
...
def _validate_latent_stage_value(cfg: DictConfig, key: str) -> None:
    ...
    target = _component_target(cfg, key)
    is_prefix_input = key == "task.prefix_input_latent" or (
        target is not None and target.endswith("Pi05PrefixInputWorldModel")
    )
    allowed = {"prefix_input"} if is_prefix_input else {"query_before", "query_after"}
    if stage is not None and stage not in allowed:
        raise ValueError(f"{key}.latent_stage must be one of {sorted(allowed)}, got {stage!r}")
...
def _matching_task_latent_stage(cfg: DictConfig, key: str) -> str | None:
    ...
    for spec_key in ("task.openvla_oft.hidden_token", "task.prefix_input_latent"):
...
def _is_chunk_world_model_target(target: str | None) -> bool:
    return target is not None and target.endswith(
        ("ChunkAwareWorldModel", "Pi05PrefixInputWorldModel")
    )
