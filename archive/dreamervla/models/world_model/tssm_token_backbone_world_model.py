...
class TSSMTokenBackboneWorldModel(DreamerV3ActorAdapterMixin):
    """Token-based TSSM WM: action_hidden kept as N=35 tokens of dim D_tok=1024.

    Key differences from the flat ``TSSMBackboneWorldModel``:
        - No ``_BackboneObsEncoder`` compressing flattened hidden→embed_dim. Instead the flattened hidden
...
__all__ = ["TSSMTokenBackboneWorldModel"]
