...
The default LIBERO data asset is OpenPI's pinned LeRobot dataset. It contains all
four no-noop suites in one repository, so there is no suite loop:
...
This writes
`${DVLA_DATA_ROOT}/datasets/lerobot/physical-intelligence/libero` from revision
`a4336d589d589045d1c56423ffdf3b88a0e19b1f`. The step uses
`https://hf-mirror.com` and clears proxy/VPN variables for the download process.

OpenVLA-OFT checkpoints remain a separate optional asset:

```bash
bash scripts/download_assets.sh download.openvla_one_traj=true only=[10_openvla_oft_one_trajectory]
```

Raw suite-by-suite HDF5 is no longer the default dataset. It remains an explicit
input to the legacy OpenVLA sidecar preprocessing route. The pinned
`libero_goal` reproduction route can prepare it with
...
This OpenVLA-only route requires raw HDF5 under
`${DVLA_DATA_ROOT}/datasets/libero/<suite>`; it does not consume the default
LeRobot dataset.
