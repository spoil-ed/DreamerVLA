    ...
    for idx in indices:
        ...
        if sequence_mode:
            ...
            if not isinstance(seq_ids, list) or not isinstance(act_seq, torch.Tensor):
                continue
            blocks = [extract_block(step_ids, block_idx=-2) for step_ids in seq_ids]
            ...
            cur_block = extract_block(sample.get("wm_obs_input_ids", []), block_idx=-2)
            next_block = extract_block(sample.get("wm_next_obs_input_ids", []), block_idx=-2)
    ...
    with torch.no_grad():
        ...
        if B >= 2:
            ...
            traj_same = torch.tensor(
                [[sample_trajectory_keys[i] and sample_trajectory_keys[i] == sample_trajectory_keys[j] for j in range(B)] for i in range(B)],
                dtype=torch.bool,
                device=device,
            ) & upper
            if task_same.any():
