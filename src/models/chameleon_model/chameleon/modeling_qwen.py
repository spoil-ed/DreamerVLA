...
@dataclass
class CausalLMOutputWithPastVisualLoss(ModelOutput):
    """
    Base class for causal language model (or autoregressive) outputs.
    ...
    hidden_states: Optional[Tuple[torch.FloatTensor, ...]] = None
    attentions: Optional[Tuple[torch.FloatTensor, ...]] = None
    visual_pred: Optional[torch.FloatTensor] = None
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: ChameleonConfig, layer_idx: int):
        ...
        print(f'{config._attn_implementation} is used in ChameleonDecoderLayer')
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # shift_embeddings = hidden_states[..., :-1, :]
            # shift_labels = labels[..., 1:]
            # loss, logits = linear_cross_entropy(shift_embeddings, self.lm_head.weight, shift_labels, return_lse=True)
        # else:
        #     logits = self.lm_head(hidden_states)
        #     logits = logits.float()
...
class RynnVLAForActionPredictionQwen(Qwen2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def __init__(self, config):
        ...
        self.action_idx = 81916

        self.state_idx = 81919
    ...
    def forward(
        ...
        if self.replace_inputs_with_action:
            ...
            state_embeds = torch.stack(state_embeds, dim=0).to(self.device).to(self.state_projection.weight.dtype)
