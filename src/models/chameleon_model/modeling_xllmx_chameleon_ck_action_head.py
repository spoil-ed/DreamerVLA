...
class Pi0StyleActionQueryHead(nn.Module):
    ...
    def forward(self, hidden_states, input_ids, attention_mask=None, target_token_id=10004, eval=False):
        ...
        for idx in range(batch_size):
            target_positions = (input_ids[idx] == target_token_id).nonzero(as_tuple=True)[0]
            if len(target_positions) > 1 or eval:
...
class ChameleonXLLMXForConditionalGeneration_ck_action_head(ChameleonForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        action_head_cls = Pi0StyleActionQueryHead if action_head_type == "pi0_query" else ActionHead
