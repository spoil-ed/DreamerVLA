...
__all__ = ["ChameleonXLLMXForConditionalGeneration_ck_action_head"]
...
class MLPResNetBlock(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.dim = dim
        self.ffn = (
            nn.Sequential(  # feedforward network, similar to the ones in Transformers
                nn.LayerNorm(dim),
                nn.Linear(dim, dim),
                nn.ReLU(),
            )
...
class L1RegressionActionHead(nn.Module):
    ...
    def predict_action(self, actions_hidden_states):
        ...
        rearranged_actions_hidden_states = actions_hidden_states.reshape(
            batch_size, self.time_horizon, -1
        )
...
class ActionHead(nn.Module):
    ...
    def extract_action_hidden(
        ...
        for i in range(batch_size):
            target_positions = (input_ids[i] == target_token_id).nonzero(as_tuple=True)[
                0
            ]
    ...
    def forward(
        ...
        for i in range(len(extracted_hidden_states)):
            ...
            combined_hidden = torch.cat(
                [extracted_hidden_states[i], action_tokens[i]], dim=0
            )
            ...
            if attention_mask is not None:
                ...
                combined_mask = torch.cat(
                    [extracted_attention_masks[i], action_tokens_mask], dim=0
                )
        ...
        for i in range(len(extracted_hidden_states)):
            ...
            if attention_mask is not None:
                ...
                if current_mask_length < max_length:
                    mask_padding = torch.zeros(
                        max_length - current_mask_length,
                        device=attention_mask.device,
                        dtype=attention_mask.dtype,
                    )
                    padded_mask = torch.cat(
                        [combined_attention_masks[i], mask_padding], dim=0
                    )
                else:
        ...
        # 堆叠成batch
        processed_hidden_states = torch.stack(
            padded_hidden_states, dim=0
        )  # [batch_size, max_length, hidden_size]
        # print(f"Processed hidden_states has NaN: {torch.isnan(processed_hidden_states).any()}")

        if attention_mask is not None:
            processed_attention_mask = torch.stack(
                padded_attention_masks, dim=0
        ...
        for i in range(len(extracted_hidden_states)):
            ...
            if action_end > transformer_output.shape[1]:
                print(
                    f"Warning: action_end ({action_end}) > sequence length ({transformer_output.shape[1]}) for batch {i}"
                )
                action_end = transformer_output.shape[1]

            # 提取action tokens对应的输出
            action_output_i = transformer_output[
                i, action_start:action_end, :
            ]  # [time_horizon * action_dim, reduced_hidden_size]
            # print(f"Batch {i}: action_output has NaN: {torch.isnan(action_output_i).any()}")
            action_outputs.append(action_output_i)

        # 将所有action outputs堆叠
        action_outputs_tensor = torch.stack(
            action_outputs, dim=0
        )  # [batch_size, time_horizon * action_dim, reduced_hidden_size]
...
class ChameleonXLLMXForConditionalGeneration_ck_action_head(
    ...
    def forward(
        self, input_ids=None, labels=None, training=False, att_mask=True, **kwargs
    ):
        if not training:
            ...
            if self.init_input_ids is None:
                ...
                self.init_input_ids = torch.cat(
                    [self.init_input_ids, input_ids], dim=-1
                )
            if not att_mask:
                attention_mask = None
            else:
        ...
        if "output_hidden_states" in kwargs:
            ...
            if not actions_flag:
                return (
                    ...
                    labels,
                    predicted_actions,
                    predicted_actions.mean() * 0,
                )

            # print(f"Predicted actions shape: {predicted_actions.shape}")
            # print(f"Predicted actions: {predicted_actions}")

            labels_action_dis, sequences = self.get_action_hs_label(
                result[2][-1], labels
            )
    ...
    def get_action_hs_label(self, hidden_states, labels_c):
        ...
        # 初始化结果张量
        labels_action = torch.zeros(
            len(sequences), self.action_dim, dtype=torch.long, device=self.device
        )
    ...
    def find_sequences(self, tensor_input):
        ...
        start_indices = (tensor_input[:, : -1 * self.action_dim + 1] == 10004).nonzero(
            as_tuple=True
        )
        valid_sequences = []
    ...
    def generate_att_mask_3(self, input_ids):
        ...
        for batch_idx in range(batch_size):
            ...
            if len(image_starts) > len(image_ends):
                ...
                image_ends = torch.cat(
                    [
                        image_ends,
                        torch.tensor(
                            [last_position], dtype=torch.long, device=self.device
                        ),
                    ]
                )
            ...
            if token == 10004:
                ...
                if start_index is not None:
                    action_sequences.append(
                        decoded_actions[start_index + 1 : end_index]
                    )
                start_index = None
    ...
    def generate_action_head(self, input_ids, generation_config):
        ...
        predicted_actions = predicted_actions.reshape(
            self.action_head.time_horizon, self.action_head.action_dim
        )
    ...
    def decode_token_ids_to_actions(self, dis_action):
        ...
        discretized_actions = torch.clamp(
            discretized_actions - 1, min=0, max=bin_centers.shape[0] - 1
        ).long()
