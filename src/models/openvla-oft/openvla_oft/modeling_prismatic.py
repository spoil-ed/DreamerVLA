    ...
    def _regression_or_discrete_prediction(
        ...
        if action_head is not None:
            ...
            if return_intermediates:
                if not hasattr(action_head, "predict_action_with_intermediates"):
                    raise AttributeError("action_head must define predict_action_with_intermediates")
                normalized_actions, hidden_c, hidden_d = action_head.predict_action_with_intermediates(
                    actions_hidden_states
                )
                return normalized_actions, actions_hidden_states, hidden_c, hidden_d
            normalized_actions = action_head.predict_action(actions_hidden_states)
            normalized_actions = normalized_actions.reshape(NUM_ACTIONS_CHUNK, ACTION_DIM)
            normalized_actions = normalized_actions.float().cpu().detach().numpy()
        else:
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if use_diffusion:
            ...
            # Run regression or discrete token-based prediction
            prediction = self._regression_or_discrete_prediction(
                input_embeddings,
                all_actions_mask,
                projected_patch_embeddings,
                attention_mask,
            ...
            if return_intermediates:
                normalized_actions, actions_hidden_states, hidden_c, hidden_d = prediction
            else:
                normalized_actions, actions_hidden_states = prediction

        # Unnormalize predicted actions
        actions = self._unnormalize_actions(normalized_actions, unnorm_key)

        if return_intermediates:
            return actions, actions_hidden_states, hidden_c, hidden_d
