...
class TSSMRynnBackboneWorldModel(DreamerV3ActorAdapterMixin):
    ...
    def __init__(
        ...
        self.encoder = _RynnBackboneObsEncoder(
            obs_dim=obs_dim, embed_dim=embed_dim,
            hidden=encoder_hidden, layers=encoder_layers, act=act,
        )

        # TSSM dynamics
        self.tssm = TSSMDynamic(
            obs_emb_dim=self.encoder.out_dim, action_dim=action_dim,
        ...
        self.rssm = self.tssm  # alias so DreamerV3ActorAdapterMixin paths that touch .rssm work

        # pixel decoder reused
        self.decoder = DreamerV3PixelDecoder(
            image_channels=image_channels, image_size=image_size,
            deter=self.tssm.deter, stoch=stoch, classes=classes, depth=depth,
            mults=tuple(mults), kernel=kernel, act=act,
        )
    ...
    def observe_next(
        self, latent: TSSMLatentState, hidden: torch.Tensor,
    ...
    def predict_next(self, latent: TSSMLatentState, actions: torch.Tensor) -> TSSMLatentState:
    ...
    def observe_sequence(self, batch: dict[str, torch.Tensor]) -> dict[str, Any]:
        ...
        latent = TSSMLatentState(deter=seq["deter"], stoch=seq["stoch"], logits=seq["post_logits"])
