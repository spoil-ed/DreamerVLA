...
Faithful port of TransDreamer's TransformerDynamic:
    /home/user01/liops/workspace/worldmodel/TransDreamer/model/modules_transformer.py
    /home/user01/liops/workspace/worldmodel/TransDreamer/model/transformer.py
...
class _MultiheadAttention(nn.Module):
    ...
    def __init__(
        self, d_model: int, n_head: int, d_inner: int,
        dropout: float, dropatt: float, pre_lnorm: bool,
    ) -> None:
        super().__init__()
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
        ...
        attn_score = torch.einsum("ibnd,jbnd->ijbn", w_q, w_k) * self.scale  # [Tq, Tk, B, H]
        if attn_mask is not None:
            attn_score = attn_score.float().masked_fill(
                attn_mask[:, :, None, None].bool(), float("-1e10"),
            ).type_as(attn_score)
...
class _PositionwiseFF(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff_inner: int, dropout: float, pre_lnorm: bool) -> None:
        super().__init__()
        self.pre_lnorm = bool(pre_lnorm)
...
class _TransformerLayer(nn.Module):
    ...
    def __init__(
        self, d_model: int, n_head: int, d_inner: int, d_ff_inner: int,
        dropout: float, dropatt: float, pre_lnorm: bool, gating: bool,
    ) -> None:
        super().__init__()
        self.mha = _MultiheadAttention(d_model, n_head, d_inner, dropout, dropatt, pre_lnorm)
...
class _Transformer(nn.Module):
    ...
    def __init__(
        self, d_model: int, n_layers: int, n_head: int, d_inner: int,
        d_ff_inner: int, dropout: float, dropatt: float,
        pre_lnorm: bool, gating: bool,
    ) -> None:
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            _TransformerLayer(d_model, n_head, d_inner, d_ff_inner,
    ...
    def forward(self, z: torch.Tensor, attn_mask: torch.Tensor | None = None) -> torch.Tensor:
    ...
    def spatio_temporal_mask(T: int, N: int, device: torch.device) -> torch.Tensor:
        ...
        """
        m = (torch.triu(torch.ones(T, T, device=device)) == 1).transpose(0, 1)  # True if t' <= t (allowed)
...
class TSSMDynamic(nn.Module):
    ...
    def __init__(
        ...
        self.cell = _Transformer(
            ...
            n_head=int(n_head), d_inner=int(d_inner),
            d_ff_inner=int(d_ff_inner),
            dropout=dropout, dropatt=dropatt,
            pre_lnorm=pre_lnorm, gating=gating,
        )

        # deter dim: concat_o → L*d_model, else d_model
        self.deter = self.n_layers * self.d_model if self.deter_type == "concat_o" else self.d_model
        ...
        self.prior_stoch_mlp = nn.Sequential(
            nn.Linear(self.deter, self.hidden), nn.ELU(),
        ...
        self.post_stoch_mlp = nn.Sequential(
            nn.Linear(self.obs_emb_dim, self.hidden), nn.ELU(),
            nn.Linear(self.hidden, self.flat_stoch),
        )
    ...
    def _build_tx_input(self, stoch: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
    ...
    def kl_loss(self, post_logits: torch.Tensor, prior_logits: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        dyn = torch.distributions.kl_divergence(self._dist(post_logits.detach()), prior).mean()
        rep = torch.distributions.kl_divergence(post, self._dist(prior_logits.detach())).mean()
        if self.free_nats > 0:
    ...
    def observe(self, enc: torch.Tensor, actions: torch.Tensor, is_first: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        B, T = enc.shape[:2]
        if T < 2:
            raise ValueError("TSSMDynamic.observe requires at least 2 timesteps for TransDreamer alignment")
        ...
        post_logits = self._logit_view(post_logits_flat)  # [B, T, stoch, classes] float32
        post_stoch = _onehot_st_sample(post_logits).to(dtype=enc.dtype)  # cast to compute dtype
        ...
        mask_s = (~target_is_first.bool()).to(dtype=prev_stoch.dtype).unsqueeze(-1).unsqueeze(-1)
    ...
    def observe_next(
        ...
        return TSSMLatentState(
            stoch=new_stoch, deter=new_deter, logits=post_logits,
...
class TSSMTokenLatentState:
    ...
    def feature(self) -> torch.Tensor:
        ...
        stoch_flat = self.stoch.reshape(*self.stoch.shape[:-2], -1)  # [..., N, stoch*classes]
...
class TSSMTokenDynamic(nn.Module):
    ...
    def __init__(
        ...
        self.token_embed = (
            nn.Identity() if self.token_dim == self.d_model else nn.Linear(self.token_dim, self.d_model)
        )
        ...
        self.cell = _Transformer(
            d_model=self.d_model, n_layers=self.n_layers,
            n_head=int(n_head), d_inner=int(d_inner), d_ff_inner=int(d_ff_inner),
            dropout=dropout, dropatt=dropatt,
            pre_lnorm=pre_lnorm, gating=gating,
        )

        # Per-token deter dim: concat_o → L*d_model, else d_model
        self.deter_per_token = self.n_layers * self.d_model if self.deter_type == "concat_o" else self.d_model
        ...
        self.post_stoch_mlp = nn.Sequential(
            nn.Linear(self.d_model, self.hidden), nn.ELU(),
            nn.Linear(self.hidden, self.flat_stoch),
        )
        # Per-token prior MLP (from deter_per_token)
        self.prior_stoch_mlp = nn.Sequential(
            nn.Linear(self.deter_per_token, self.hidden), nn.ELU(),
            nn.Linear(self.hidden, self.flat_stoch),
        )
    ...
    def _build_tx_input(
        self, prev_stoch_flat: torch.Tensor, prev_action: torch.Tensor,
    ) -> torch.Tensor:
        """
    ...
    def observe(
        self, obs_tokens: torch.Tensor, actions: torch.Tensor, is_first: torch.Tensor,
        ...
        if T < 2:
            raise ValueError("TSSMTokenDynamic.observe requires at least 2 timesteps for TransDreamer alignment")
        ...
        steps = T - 1
        target_is_first = is_first[:, 1:]
        m_s = (~target_is_first.bool()).to(dtype=prev_stoch.dtype).view(B, steps, 1, 1, 1)
        ...
        tx_in = self._build_tx_input(prev_stoch_flat, prev_action)  # [B, (T-1)*N, d_model]
        if steps > self.tssm_window:
            # truncate to last `tssm_window` timesteps (window*N tokens), pad deter for earlier steps
            tx_in_trunc = tx_in.reshape(B, steps, N, self.d_model)[:, -self.tssm_window:].reshape(B, self.tssm_window * N, self.d_model)
    ...
    def observe_next(
        ...
        new_stoch = _onehot_st_sample(post_logits).to(dtype=obs_tokens.dtype)  # [B, N, stoch, classes]
...
def _build_hidden_decoder(
    kind, in_dim, out_dim, *,
    layers, units, d_model, nhead, mem_tokens, dropout, act,
    n_time_queries: int = 5, joint_broadcast: int = 7,
):
    kind = str(kind).lower()
    if kind == "mlp":
    ...
    if kind in {"pi0_transformer", "transformer", "pi0"}:
        return Pi0StyleHiddenDecoder(
            in_dim, out_dim, layers=layers, d_model=d_model, nhead=nhead,
            mem_tokens=mem_tokens, dropout=dropout, act=act,
        )
    if kind in {"pi0_time_broadcast", "time_broadcast", "pi0_time"}:
        # Lazy import to avoid circular: lives in dreamerv3_torch.
        from src.models.world_model.dreamerv3_torch import Pi0TimeBroadcastDecoder
        return Pi0TimeBroadcastDecoder(
            in_dim, out_dim, layers=layers, d_model=d_model, nhead=nhead,
            mem_tokens=mem_tokens, n_time_queries=int(n_time_queries),
            joint_broadcast=int(joint_broadcast), dropout=dropout, act=act,
        )
...
class _PerTokenMLPDecoder(nn.Module):
    ...
    """
    def __init__(self, in_dim_per_tok: int, out_dim_per_tok: int, n_tokens: int,
                 layers: int = 2, units: int = 4096, act: str = "silu") -> None:
        super().__init__()
        self.n_tokens = int(n_tokens)
        self.out_dim_per_tok = int(out_dim_per_tok)
        self.mlp = MLPHead(int(in_dim_per_tok), int(out_dim_per_tok),
                           layers=int(layers), units=int(units), act=act)
...
_WORLD_MODEL_EXPORTS = {
    "TSSMRynnBackboneWorldModel": "src.models.world_model.tssm_rynn_backbone_world_model",
    "TSSMTokenRynnBackboneWorldModel": "src.models.world_model.tssm_token_rynn_backbone_world_model",
}
