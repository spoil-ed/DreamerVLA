"""LatentSuccessClassifier training workspace (WMPO-aligned).

Three-layer architecture per CLAUDE.md:
    scripts/train_latent_classifier.sh           # env-var wrapper
        → python -m src.cli.train --config-name latent_classifier_libero_goal
            → src.workspace.LatentClassifierWorkspace.run()
                → src.dataloader.wmpo_aligned_latent_dataset (1 pos + 1 neg per demo)
                → src.models.reward.LatentSuccessClassifier (linear | mlp2 | transformer)

Why a dedicated workspace, not another standalone script:
  * The existing v3 / wm_replay classifier scripts are 500+ lines each and bypass
    BaseWorkspace, so resume / checkpoint / Hydra-override semantics don't
    transfer. A workspace fixes that.
  * Decouples sampling protocol (dataset), model (LatentSuccessClassifier head_type),
    and training loop (this workspace) — so head_type ablation is a 1-line config
...
from src.dataloader.wmpo_aligned_latent_dataset import (
...
class LatentClassifierWorkspace(BaseWorkspace):
    ...
    Lifecycle: setup() → run() → teardown() (via BaseWorkspace.execute).
    """
    ...
    # ----------------------- BaseWorkspace exclusions ------------------

    # Datasets carry the in-memory demo cache (multi-GB); keep them out of
    # the BaseWorkspace state_dict serializer so save_checkpoint doesn't try
    # to pickle them.
...
__all__ = ["LatentClassifierWorkspace"]
