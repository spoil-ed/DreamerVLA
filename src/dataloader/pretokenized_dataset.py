...
class PretokenizedRynnVLADataSpec:
    manifest_path: str
    cache_dir: str
    num_samples: int
    num_shards: int
...
class PretokenizedRynnVLADataset(BaseDataset):
    def __init__(
        self,
        manifest_path: str | Path = "/home/yuxinglei/workspace/2026nips/Dreamer-VLA/data/pretokenized/rynnvla_libero_object/manifest.pt",
        max_open_shards: int = 2,
    ) -> None:
        super().__init__()
        self.manifest_path = Path(manifest_path).expanduser().resolve()
        if not self.manifest_path.is_file():
            raise FileNotFoundError(f"Pretokenized manifest does not exist: {self.manifest_path}")
        ...
        self._data_spec = PretokenizedRynnVLADataSpec(
    ...
    @property
    def data_spec(self) -> PretokenizedRynnVLADataSpec:
...
__all__ = ["PretokenizedRynnVLADataSpec", "PretokenizedRynnVLADataset"]
