...
def process_libero_data(
    ...
    for task_id, task in enumerate(task_list):
        ...
        for i, trj in enumerate(trj_list):
            ...
            image_maps = [_collect_indexed_files(imgs_path, "image_", ".png") for imgs_path in imgs_paths]
            state_map = _collect_indexed_files(state_path, "eef_gripper_state_", ".npy") if with_state else {}
            ...
            for j in range(len(action_list)):
                ...
                reward_value = float(reward_list[min(j + len_action - 1, len(reward_list) - 1)])
                ...
                if with_state:
                    ...
                    world_human_val = prompt_text + "<|state|>" * len(world_state) + "<|image|>" * len(
                        world_input_images
                    ) + "<|action|>" * len(world_action)
                else:
                    world_state = []
                    world_human_val = prompt_text + "<|image|>" * len(world_input_images) + "<|action|>" * len(
                        world_action
                    )
...
def main():
    ...
    parser.add_argument(
        '--with_world_model', action='store_true',
