...
do
    ...
    echo "Executing: python -u ${SCRIPT_DIR}/concate_record.py --sub_record_dir ${sub_record_dir} --save_path ${save_path}"
    
    # Directly execute the command, which is safer than using eval
    python -u "${SCRIPT_DIR}/concate_record.py" --sub_record_dir "${sub_record_dir}" --save_path "${save_path}"
