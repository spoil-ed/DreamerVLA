...
driven by ``ChunkAwareRynnDinoWMWorldModel.predict_next_chunk`` so each
...
Per chunk c we record:
    ...
    first_action[c], old_lp[c]      — sampled first action + chunk-level log_prob
    rewards[c, :, k]                — dense state-reward at imagined frame k

PPO ratio is computed on the chunk-level log_prob (one per chunk), and the
return is the γ-discounted sum of all horizon * K per-frame rewards. GRPO
group-relative advantage is broadcast across all chunks of a rollout.
...
def dino_wmpo_dense_chunk_step(
    ...
    if real_relabel_batch is not None and float(
    ...
    first_actions: list[torch.Tensor] = []     # [B_eff, A] per chunk — basis for PPO ratio
    ...
    with _temporarily_freeze(chunk_world_model):
        for _ in range(horizon):
            ...
            with torch.no_grad():
                first_action, old_lp, sample_extra = policy({
                    "mode": "sample", "hidden": actor_feat, "deterministic": False,
                })
                full_chunk = sample_extra.get("action_chunk")
            if full_chunk is None:
                raise RuntimeError(
                    "Pi0ActionHiddenActor sample-mode output missing 'action_chunk' in extras; "
                    "dense_chunk requires the actor to emit its full K-step chunk."
                )
            if full_chunk.ndim != 3 or full_chunk.shape[1] != K:
                raise ValueError(
                    f"action_chunk shape mismatch: got {tuple(full_chunk.shape)}, expected [B,K={K},action_dim]"
                )
            # Splice the stochastically sampled first action into the deterministic chunk
            # so the WM rollout is consistent with the action whose log_prob drives PPO.
            action_chunk = full_chunk.clone()
            action_chunk[:, 0, :] = first_action

            actor_feats.append(actor_feat)
            first_actions.append(first_action.detach())
            old_log_probs.append(old_lp.detach())

            if use_ref:
                with torch.no_grad():
                    ref_lp, _, _ = ref_policy({
                        "mode": "evaluate", "hidden": actor_feat, "action": first_action.detach(),
                    })
    ...
    for _ in range(update_epochs):
        ...
        for actor_feat, action_detached in zip(actor_feats, first_actions, strict=True):
            new_lp, entropy_t, _ = policy({
                "mode": "evaluate", "hidden": actor_feat, "action": action_detached,
            })
        ...
        ent_loss = -(entropy_coef * entropy_stack.sum(dim=1)).mean() if entropy_coef else pg_loss.new_zeros(())
    ...
    return {
        # Flat keys — matched to dreamer_vla_workspace metric extraction so
        ...
        "dense_chunk/actor_grad_norm_output_projection": _named_grad_norm(policy, "output_projection"),
