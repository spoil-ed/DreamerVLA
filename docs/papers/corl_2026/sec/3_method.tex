...
the projected visual hidden tokens produced before the language-model action
...
world model that predicts future hidden-token grids, (ii) a spatial success
classifier trained on windows of the same token grids, and (iii) an actor bridge
that maps predicted hidden tokens to OpenVLA-OFT's internal discrete action
...
shape-aligned hidden-token sidecar.  Before reuse, the loader verifies the source
...
\subsection{Hidden-Token Observation Contract}
