...
主线依次执行：真实 LIBERO rollout 采集、回放初始化、世界模型与分类器 warm-up、在线协同训练和真实环境评估。reward shard 与 input-token sidecar 必须具有相同的文件集合、demo 集合和帧长度。系统在续采、回放初始化以及两个独立训练入口之前验证所有 demo 的形状。
