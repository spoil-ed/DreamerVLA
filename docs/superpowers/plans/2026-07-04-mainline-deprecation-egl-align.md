...
**Step 2 — EGL 三处对齐（R3）**
- 先实证主线 collect 真实渲染入口（`ColdStartRayCollectRunner` vs
  `collect_parallel_rollouts.py:497`）；再把 collect/cotrain-real/eval 三处默认改 egl；
  移植/复用 per-worker 设备绑定；保留 osmesa 回退与零 GPU 拒绝。
- verify：三处默认值 = egl；有 GPU 时 `render_backend=egl` 冒烟不崩；无 GPU 标 GPU-GATED
  并静态确认 device-binding 代码路径。
