...
- 相关文档:
  - 原始设计:`docs/superpowers/specs/2026-06-16-parallel-rollout-collection-design.md`
  - 实现计划:`docs/superpowers/plans/2026-06-16-parallel-rollout-collector.md`
- 相关代码(本仓):
...
- **Ray(后续整体 online loop 的扩展选项)**:本采集器单机 torchrun 足够,**不引入 Ray**;
  但**完整在线 cotrain loop**(policy + replay + learner,见
  `docs/superpowers/specs/2026-06-17-offline-warmup-online-cotrain-pipeline-design.md`)
  在更大规模下,Ray 可作架构选项:
  - **异构 worker 放置**:推理 actor(VLA encoder + WM 前向产 latent/action)、env worker、
    learner(WM/classifier/RL 反传)分置不同设备/进程,各按吞吐独立扩缩。
  - **infer-step-learner 流水线重叠**:rollout 推理、env step、learner 更新三者重叠以掩盖
    互等(采集器内部的 infer/step 重叠用双缓冲即可,不必 Ray;此处针对整体 loop)。
  - **多节点**:replay 作为共享/分片服务,多机 env worker + 多机 learner(DDP/FSDP)。
  - **立场**:单机 torchrun/DDP/FSDP 仍为主线(`AGENTS.md:6`、`AGENTS.md:69`);Ray 仅作
    整体 loop 的**未来架构选项**记录,本期不实现、不引入依赖。
...
- **采集器 Hydra 化 + 冷启动 WM 消费对齐**:见
  `docs/superpowers/specs/2026-06-17-coldstart-collector-hydra-and-wm-consumption.md`
