...
The ignored `third_party/` checkouts must exist at the revisions documented in
...
env -u HTTP_PROXY -u HTTPS_PROXY -u ALL_PROXY \
    -u http_proxy -u https_proxy -u all_proxy \
    uv sync --locked --all-extras --group cu124-train
```
...
`--all-extras` selects the maintained `ray`, `libero`, and `openvla` extras.
