...
bash scripts/experiments/world_model_training/train.sh \
  ...
  training.out_dir=/path/to/world_model/run
...
| [experiment/dreamervla_wmcls_cotrain_ray.yaml](../../../configs/experiment/dreamervla_wmcls_cotrain_ray.yaml) | WM/CLS 可训练，训练与评估解耦 |
...
```bash
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
  COTRAIN_RESUME_CKPT="${RUN_ROOT}/cotrain/checkpoints/manual_cotrain_step_10/manual_cotrain.ckpt" \
  bash scripts/experiments/cotrain/train.sh
```
...
```bash
python -m dreamervla.diagnostics.wandb_relay_sync --dry-run --once
python -m dreamervla.diagnostics.wandb_relay_sync --once
```
