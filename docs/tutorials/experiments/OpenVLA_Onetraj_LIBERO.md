...
```bash
WORLD_MODEL_CKPT=/path/to/world_model.ckpt \
CLASSIFIER_CKPT=/path/to/classifier.ckpt \
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
bash scripts/experiments/cotrain/train.sh
```

该入口选择 `experiment=dreamervla_wmcls_cotrain` 和 public
...
| `configs/experiment/dreamervla_wmcls_cotrain.yaml` | 完整 cotrain recipe |
