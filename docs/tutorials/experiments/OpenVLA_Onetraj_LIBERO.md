...
RealEnvWorker ---- observations ----+
                                    |
WMEnvWorker -- hidden_token [256,4096] -+--> RolloutGroup -- trajectory --> ActorGroup
     ^                                      ^                              |
...
从显式 checkpoint 加载后保持冻结；非冻结主线则由 LearnerGroup 在一张卡上更新并
同步 WM/CLS。两条路线在 WM forward、classifier forward、Rollout policy 和
Actor trajectory 边界都保留 `hidden_token [256,4096]`，内部扁平存储不会作为
跨组件接口暴露。
...
manual global step 明确包含 4 次 optimizer step，随后才执行 Learner update 和权重
同步。
...
7. ActorGroup 将 trajectory flatten/shuffle，按 global batch 16384、每 rank micro
   batch 32 执行 FSDP PPO forward/backward，并完成 4 次 optimizer step。
