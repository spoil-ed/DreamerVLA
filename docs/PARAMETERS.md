...
| Key | Meaning |
| --- | --- |
| `world_model.token_count` | fixed prefix length, `968` |
| `world_model.token_dim` | native PaliGemma input width, `2048` |
| `world_model.image_token_count` | dynamic visual slots, fixed at `768` |
| `world_model.text_token_count` | static language slots, fixed at `200` |
| `world_model.text_mode` | `exact` keeps native language embeddings/mask; `masked` zeros the slots and mask without changing shape |
| `world_model.wm_use_text` | whether the WM observes language slots |
| `world_model.policy_use_text` | whether native PaliGemma/action-expert prefill observes language slots |
| `world_model.predict_text_tokens` | must remain `false`; language has no dynamic reconstruction target |

This opt-in contract is selected by `experiment=wm_pi05_prefix_input_train` and
is distinct from the existing post-transformer `pi05_prefix_output [768,2048]`
route. See [`pi05_prefix_input_latent.md`](pi05_prefix_input_latent.md).
