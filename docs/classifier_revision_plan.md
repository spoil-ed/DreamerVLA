...
- [ ] **1.1 新数据集** `src/dataloader/wm_replay_classifier_dataset.py::WMReplayClassifierDataset`
...
- [ ] **2.1 模型选项** 在 [latent_success_classifier.py](../src/models/reward/latent_success_classifier.py) 加 `head_type: Literal["transformer8", "mlp2", "linear"]`,linear 走 `nn.Linear(latent_dim*W, 2)`,mlp2 走 `[Linear(L*W, 1024), GELU, Linear(1024, 2)]`
