...
`task.pi05.ckpt_path`. The base contains `model.safetensors`; normalization is
...
three native `embed_image()` blocks occupy slots `0:768`, and the original 200
tokenizer/embedding slots occupy `768:968`. The WM predicts and reconstructs only
the 768 visual slots. It retains language as static conditioning metadata so the
policy path can rebuild an exact or masked prefix, run native PaliGemma prefill,
and pass the resulting KV cache to the original π0.5 flow-matching expert. This
does not change `wm_pi05_collected_train` or its post-transformer image-only
